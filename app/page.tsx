...
export default function Home() {
  ...
  useEffect(() => {
    ...
    console.log('[Home] window.electronAPI:', !!window.electronAPI);
    ...
    const handleQuickInput = async (message: unknown) => {
      console.log('[Home] ===== Received quick input message =====');
      console.log('[Home] Message:', message);
      console.log('[Home] Message type:', typeof message);

      if (typeof message !== 'string') {
        console.error('[Home] Invalid message type');
        return;
      }
      ...
      try {
        ...
        // 새 대화 활성화
        console.log('[Home] Activating conversation...');
        await setActiveConversation(conversationId);
        console.log('[Home] Conversation activated');

        // UI가 업데이트될 시간을 주고 메시지 전송
        setTimeout(() => {
          console.log('[Home] Dispatching custom event with message:', message);
          window.dispatchEvent(
            new CustomEvent('sepilot:quick-input-message', {
              detail: { message },
            })
          );
          console.log('[Home] Custom event dispatched');
        }, 200);
      } catch (error) {
    ...
    // IPC 이벤트 리스너 등록
    console.log('[Home] Registering IPC listener for: create-new-chat-with-message');
    window.electronAPI.on('create-new-chat-with-message', handleQuickInput);
    console.log('[Home] IPC listener registered');

    return () => {
      console.log('[Home] Cleaning up IPC listener');
