...
import { z } from 'zod';
import type { SkillPackage, SkillManifest, SkillContent } from '../../types/skill';
...
const SkillManifestSchema = z.object({
  ...
  id: z
    ...
    .regex(/^[a-z][a-z0-9]*(\.[a-z][a-z0-9]*)*$/, {
      message: 'ID must follow reverse domain notation (e.g., com.example.skill)',
    }),
...
export class SkillValidator {
  ...
  private validateManifest(manifest: SkillManifest): ValidationResult {
    ...
    try {
      ...
      if (manifest.contextPatterns && manifest.contextPatterns.length > 20) {
        result.warnings.push(`Too many context patterns: ${manifest.contextPatterns.length} (recommended: ≤20)`);
      }
  ...
  private validateContent(content: SkillContent): ValidationResult {
    ...
    if (content.knowledge) {
      if (content.knowledge.length > 50) {
        result.warnings.push(`Too many knowledge sections: ${content.knowledge.length} (recommended: ≤50)`);
      }
        ...
        if (template.variables) {
          ...
          for (const variable of template.variables) {
            if (varNames.has(variable.name)) {
              result.warnings.push(`Duplicate variable name in template ${template.id}: ${variable.name}`);
            }
   ...
   * Resources 검증
   */
  private validateResources(resources: any): ValidationResult {
    const result: ValidationResult = {
      valid: true,
      errors: [],
      warnings: [],
    ...
    if (resources.files) {
      for (let i = 0; i < resources.files.length; i++) {
        ...
        if (!file.filename || !file.content) {
          result.errors.push(`File ${i} missing filename or content`);
        }
        ...
        if (fileSize > this.options.maxResourceSize) {
          result.errors.push(
            `File ${file.filename} too large: ${fileSize} bytes (max: ${this.options.maxResourceSize})`
          );
        }
      }
    }
    ...
    if (resources.images) {
      for (let i = 0; i < resources.images.length; i++) {
        ...
        if (!image.filename || !image.data) {
          result.errors.push(`Image ${i} missing filename or data`);
        }
        ...
        if (imageSize > this.options.maxResourceSize) {
          result.errors.push(
            `Image ${image.filename} too large: ${imageSize} bytes (max: ${this.options.maxResourceSize})`
          );
        }

        // MIME type 확인
        if (!image.mimeType || !image.mimeType.startsWith('image/')) {
          result.warnings.push(`Image ${image.filename} has invalid MIME type: ${image.mimeType}`);
        }
      }
    }
