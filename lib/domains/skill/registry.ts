...
interface SkillMutationOptions {
...
export class SkillRegistry {
  ...
  async installSkill(
    ...
    try {
      ...
      const trustLevel = this.resolveTrustLevel(source, options);

      await this.validateSkillPackage(skillPackage, trustLevel);
      ...
      const newError = new Error(`Failed to install skill: ${errorMessage}`);
  ...
  async updateSkill(
    skillPackage: SkillPackage,
    options: SkillMutationOptions & { sourceOverride?: SkillSource } = {}
  ): Promise<InstalledSkill> {
    const { skillDatabaseService } = await import('../../../electron/services/skill-database');
    const { skillStorageService } = await import('../../../electron/services/skill-storage');
    ...
    try {
      ...
      const trustLevel = this.resolveTrustLevel(existingSkill.source, options);
      await this.validateSkillPackage(skillPackage, trustLevel);
      ...
      const updatedSkill: InstalledSkill = {
        ...existingSkill,
        manifest: skillPackage.manifest,
        source: options.sourceOverride ?? existingSkill.source,
        localPath,
      };
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const newError = new Error(`Failed to update skill: ${errorMessage}`);
      if (error instanceof Error) {
        (newError as any).cause = error;
      }
      throw newError;
  ...
  async removeSkill(skillId: string): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const newError = new Error(`Failed to remove skill: ${errorMessage}`);
      if (error instanceof Error) {
        (newError as any).cause = error;
      }
      throw newError;
  ...
  async enableSkill(skillId: string): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const newError = new Error(`Failed to enable skill: ${errorMessage}`);
      if (error instanceof Error) {
        (newError as any).cause = error;
      }
      throw newError;
  ...
  async disableSkill(skillId: string): Promise<void> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      const newError = new Error(`Failed to disable skill: ${errorMessage}`);
      if (error instanceof Error) {
        (newError as any).cause = error;
      }
      throw newError;
  ...
  private resolveTrustLevel(source: SkillSource, options: SkillMutationOptions): SkillTrustLevel {
    if (options.trustLevel) {
      return options.trustLevel;
    }
    return source.type === 'builtin' ? 'trusted' : 'untrusted';
  }

  private async validateSkillPackage(
    skillPackage: SkillPackage,
    trustLevel: SkillTrustLevel
  ): Promise<void> {
    const { skillLoader } = await import('./loader');
    const { SkillValidator } = await import('./validator');
    ...
    const validator = new SkillValidator({
      checkSecurity: trustLevel === 'untrusted',
      strictMode: false,
    });

    const validation = await validator.validate(skillPackage);
    if (!validation.valid) {
      const errors = [...validation.errors, ...validation.securityIssues];
      const details = errors.slice(0, 5).join('; ');
      throw new Error(`Skill validation failed (${trustLevel}): ${details}`);
    }
  }
}
