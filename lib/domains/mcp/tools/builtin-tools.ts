...
import { getActiveBrowserView } from '../../../../electron/ipc/handlers/browser/browser-control';
import {
 ...
 */
function resolvePath(filePath: string, conversationId?: string): string {
  const workingDir = getCurrentWorkingDirectory(conversationId);
 ...
 *
 * 도구 설명에 현재 OS/셸 정보를 포함하여 LLM이 올바른 명령어 문법을 생성하도록 합니다.
 */
function getCommandExecuteDescription(): string {
  const isWindows = process.platform === 'win32';
  if (isWindows) {
    return 'Execute shell commands via cmd.exe (Windows). Use Windows CMD syntax: set VAR=value, %VAR%, backslash paths. Do NOT use bash syntax (export, $VAR, forward slash paths). Python venv uses Scripts\\python.exe (not bin/python).';
  }
  return 'Execute shell commands via bash. Use standard Unix syntax.';
}

function getCommandDescription(): string {
  const isWindows = process.platform === 'win32';
  if (isWindows) {
    return 'Windows CMD command to execute. Use CMD syntax: "set VAR=value && echo %VAR%", "dir", "type file.txt". Python: "%USERPROFILE%\\.sepilot\\environments\\python\\Scripts\\python.exe" script.py';
  }
  return 'Shell command to execute (e.g., "npm install", "git status")';
}

export const commandExecuteTool: MCPTool = {
  name: 'command_execute',
  description: getCommandExecuteDescription(),
  serverName: 'builtin',
  inputSchema: {
    type: 'object',
    properties: {
...
export async function executeBuiltinTool(
  ...
  switch (toolName) {
    case 'file_read':
      return await handleFileRead(args as { path: string }, conversationId);
    case 'file_write':
      return await handleFileWrite(args as { path: string; content: string }, conversationId);
    case 'file_edit':
      return await handleFileEdit(
        args as { path: string; old_str: string; new_str: string },
        conversationId
      );
    case 'file_list':
      return await handleFileList(args as { path?: string; recursive?: boolean }, conversationId);
    case 'command_execute':
      return await handleCommandExecute(args as { command: string; cwd?: string }, conversationId);
    case 'grep_search':
 ...
 * Handle file_read
 */
async function handleFileRead(args: { path: string }, conversationId?: string): Promise<string> {
  try {
    const absolutePath = resolvePath(args.path, conversationId);
...
async function handleFileList(
  ...
  try {
    const absolutePath = resolvePath(dirPath, conversationId);
...
function validateCommand(command: string): void {
  ...
  const dangerousPatterns = [
    ...
    // Shell control characters (newlines can inject hidden commands)
    /[\r\n]/,

    // Backtick command substitution with dangerous commands
    /`[^`]*\b(rm|dd|mkfs|shutdown|reboot|curl|wget|nc)\b[^`]*`/i,

    // $() command substitution with dangerous commands
    /\$\([^)]*\b(rm|dd|mkfs|shutdown|reboot|curl|wget|nc)\b/i,

    // Semicolon chaining with dangerous commands (not already covered above)
    /;\s*(shutdown|reboot|dd\s|mkfs|nc\s)/i,

    // Redirection to sensitive system paths
    />\s*\/etc\//i,
    />\s*\/dev\/(?!null)/i,
    />\s*~\/\.ssh\//i,
    />\s*~\/\.(bashrc|profile|bash_profile|zshrc)/i,
  ];
...
async function handleCommandExecute(
  ...
  try {
    ...
    const currentWorkingDir = getCurrentWorkingDirectory(conversationId);
    ...
    const workingDir = cwdArg ? resolvePath(cwdArg, conversationId) : currentWorkingDir!;
...
async function handleGrepSearch(
  ...
  try {
    ...
    const absolutePath = resolvePath(searchPath, conversationId);
    ...
    // File type filter
    if (args.file_type) {
      rgArgs.push('-t', args.file_type);
    }

    // Show line numbers and file names
    rgArgs.push('-n', args.pattern, absolutePath);
    ...
    const { stdout, stderr } = await execFilePromise('rg', rgArgs, {
      cwd: workingDir,
      maxBuffer: 1024 * 1024 * 10, // 10MB buffer
...
async function handleBrowserCreateTab(args: { url?: string }): Promise<string> {
  try {
    const { browserCreateTab } =
      await import('../../../../electron/ipc/handlers/browser/browser-view');
