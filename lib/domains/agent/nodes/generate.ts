...
import { getBuiltinTools } from '@/lib/domains/mcp/tools/builtin-tools';
  ...
  try {
    ...
    if ((llmConfig as any).connections) {
      ...
      if (isLLMConfigV2(llmConfig)) {
        logger.info('[getLLMConfigFromDB] Converting V2 config', {
          activeBaseModelId: llmConfig.activeBaseModelId,
        });
        llmConfig = convertV2ToV1(llmConfig);
      }
    }

    logger.info('[getLLMConfigFromDB] Extracted config:', {
...
export async function* generateNode(state: ChatState): AsyncGenerator<Partial<ChatState>> {
  try {
    ...
    // Prefer state-scoped conversationId for concurrent stream safety.
...
export async function generateWithToolsNode(state: AgentState): Promise<Partial<AgentState>> {
  try {
    ...
    if (hasToolAllowlist) {
      availableTools = availableTools.filter((tool) => enabledToolNames.has(tool.name));
      logger.info('[Agent] Applied enabledTools allowlist:', {
        requested: graphConfig?.enabledTools,
        remaining: availableTools.map((tool) => tool.name),
      });
    }
...
선택해주세요!"`;
    ...
    const toolCalls = finalToolCalls
      ?.filter((tc: any) => {
        ...
        if (!allowed) {
          logger.warn('[Agent] Ignoring tool call not in allowed tool list:', {
            toolName,
          });
        }
        return allowed;
        ...
        return {
          ...
          arguments: (() => {
            try {
              ...
              logger.error('[Agent] Failed to parse tool arguments:', {
                error: e,
                args: tc.function.arguments,
              });
              // Return empty object on failure to prevent crash,
              // the tool execution will likely validate and fail gracefully later
              return {};
            }
          })(),
        };
