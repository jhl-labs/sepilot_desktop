...
import { generateWithNanoBanana } from '@/lib/domains/integration/imagegen/nanobanana-client';
import { httpPost, httpFetch, createWebSocket } from '@/lib/http';

import { logger } from '@/lib/utils/logger';

const COMFY_DIMENSION_MIN = 256;
const COMFY_DIMENSION_MAX = 2048;
const COMFY_DIMENSION_STEP = 8;
const COMFY_DEFAULT_WIDTH = 512;
const COMFY_DEFAULT_HEIGHT = 512;

function normalizeComfyDimension(value: unknown, fallback: number): number {
  const numericValue =
    typeof value === 'number'
      ? value
      : typeof value === 'string'
  ...
  const rounded = Math.round(numericValue / COMFY_DIMENSION_STEP) * COMFY_DIMENSION_STEP;
  return Math.min(COMFY_DIMENSION_MAX, Math.max(COMFY_DIMENSION_MIN, rounded));
}
...
async function generateImageInMainProcess(
  ...
  try {
    const width = normalizeComfyDimension(args.width, COMFY_DEFAULT_WIDTH);
    const height = normalizeComfyDimension(args.height, COMFY_DEFAULT_HEIGHT);
    const defaultPositivePrompt = config.positivePrompt?.trim();
    const positivePrompt = defaultPositivePrompt
      ? `${defaultPositivePrompt}, ${args.prompt}`
      : args.prompt;
    ...
    const workflow = {
      ...
      '75:6': {
        inputs: { text: positivePrompt, clip: ['75:38', 0] },
        class_type: 'CLIPTextEncode',
...
export async function toolsNode(state: AgentState): Promise<Partial<AgentState>> {
  try {
    ...
    const isToolAllowed = (toolName: string): boolean => {
      ...
      // generate_image is controlled by separate ImageGen toggle
      if (toolName === 'generate_image') {
        return graphConfig?.enableImageGeneration === true;
      }
      ...
      lastMessage.tool_calls.map(async (call) => {
        try {
          ...
          if (!isToolAllowed(call.name)) {
            logger.warn(`[Tools] Blocked disabled tool call: ${call.name}`);
            return {
              toolCallId: call.id,
              toolName: call.name,
              result: null,
          ...
          if (call.name === 'generate_image') {
            const imageGenConfig = getCurrentImageGenConfig(state.conversationId);
            const networkConfig = getCurrentNetworkConfig(state.conversationId);
