...
export class ComfyUIClient {
  ...
  private buildWorkflow(options: ComfyUIGenerateOptions): Record<string, any> {
    ...
    const width = normalizeComfyDimension(options.width, COMFY_DEFAULT_WIDTH);
    const height = normalizeComfyDimension(options.height, COMFY_DEFAULT_HEIGHT);
    const defaultPositivePrompt = this.config.positivePrompt?.trim();
    const positivePrompt = defaultPositivePrompt
      ? `${defaultPositivePrompt}, ${options.prompt}`
      : options.prompt;
