...
- **Markdown 파일에서 클립보드 이미지 붙여넣기 기능**
  ...
  - **Markdown 문법 자동 삽입**: 이미지가 저장되면 `![filename](relativePath)` 형식으로 커서 위치에 자동 삽입
  - **상대 경로 자동 계산**: 현재 Markdown 파일의 위치를 기준으로 이미지 경로 자동 계산
  - **미리보기 렌더링**: MarkdownRenderer가 상대 경로를 절대 경로로 자동 변환하여 미리보기에서 이미지 표시
  ...
  - 코드 위치:
    - IPC 핸들러: `electron/ipc/handlers/file.ts:712-767`
    - 클라이언트: `components/editor/Editor.tsx:820-898`
    - 렌더러: `components/markdown/MarkdownRenderer.tsx:124-182`
