...
* **Monaco Editor CSP 위반 문제 해결**
  * Monaco Editor가 CDN에서 스크립트를 로드하려다 CSP 정책에 의해 차단되던 문제 수정
  * loader 설정으로 로컬 파일 경로 지정: `/monaco-editor/min/vs`
  * node_modules의 Monaco 파일을 public 디렉토리로 복사
  * postinstall 스크립트(`scripts/copy-monaco.js`)로 자동화
  * .gitignore에 Monaco 파일 추가 (빌드 시 자동 생성)
