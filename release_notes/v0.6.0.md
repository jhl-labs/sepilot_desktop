...
* **Editor 모드 추가 (Chat + Editor 듀얼 모드)**
  ...
  * **전체 파일 검색 (Ctrl+Shift+F)**
    ...
    - 파일별 그룹화 및 확장/축소 UI
    - 검색 결과 개수 표시 (파일/매칭 수)
    - node_modules, .git, dist, build 등 자동 제외
  * Monaco Editor 통합:
...
* **컴포넌트 테스트 대규모 수정 및 커버리지 개선**
  * Sidebar 테스트 완전 재작성 (appMode 기반 구조 반영)
  ...
  * 전체 테스트 커버리지: 41.39% → 42.51%
  * 실패 테스트: 7개 스위트 36개 실패 → 1개 스위트 2개 실패
  * 통과 테스트: 649개 → 683개 (+34개)
