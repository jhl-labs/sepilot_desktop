...
* **Editor 모드 추가 (Chat + Editor 듀얼 모드)**
  ...
  * **Browser Agent 통합 (AI 기반 웹 브라우징 자동화)**
    ...
    - **BrowserAgentGraph** 클래스 구현
      ...
      * 최대 15회 반복 (복잡한 브라우저 작업을 위해 일반 Agent보다 높게 설정)
      * Human-in-the-loop 도구 승인 지원
      * 브라우저별 로깅 및 오류 처리
...
* **Editor에 LLM 기반 자동완성 및 컨텍스트 메뉴 기능 추가**
  * **자동완성용 별도 모델 설정 지원**
    ...
    - Temperature (기본 0.3), Max Tokens (기본 100), Debounce (기본 300ms) 설정
    - 자동완성 On/Off 토글 기능
...
* **Browser 모드 전환 시 BrowserView가 계속 표시되는 문제 수정**
  ...
  * **추가 개선 (3차) - Settings/Documents/Gallery에서도 숨김**:
    - BrowserView는 네이티브 오버레이라서 z-index로 제어 불가
    - Settings 다이얼로그 열릴 때 BrowserView 임시 숨김
    - Documents/Gallery 페이지 열릴 때도 BrowserView 숨김
    - 닫힐 때 browser 모드이고 chat 모드이면 BrowserView 다시 표시
    - Browser 모드에서도 Settings가 정상적으로 표시됨
