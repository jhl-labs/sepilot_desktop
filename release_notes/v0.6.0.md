...
* **Editor 모드 추가 (Chat + Editor 듀얼 모드)**
  ...
  * **Editor 설정 UI (Sidebar 내 인라인 뷰)**
    ...
    - **외형 설정**
      * 폰트 크기: 10-24px (기본값: 14px)
      * 폰트 종류: 9가지 모노스페이스 폰트 (Consolas, Monaco, Menlo, Courier New, Fira Code, Source Code Pro, JetBrains Mono, D2Coding, Nanum Gothic Coding)
      * 테마: Dark/Light (기본값: Dark)
      * 탭 크기: 2/4/8 spaces (기본값: 2)
      * 자동 줄바꿈: On/Off (기본값: Off)
      * 미니맵 표시: On/Off (기본값: On)
      * 줄 번호 표시: On/Off (기본값: On)
    - **LLM 프롬프트 커스터마이징**
      * 자동 완성 프롬프트
      * 코드 설명 프롬프트
      * 리팩토링 프롬프트
...
* **사용 가능한 툴 목록 표시 기능**
  ...
  * **Chat/Browser/Editor 모드별 도구 분리**
    ...
    - Browser: Browser Agent 전용 도구 (browser_* 12개 도구)
