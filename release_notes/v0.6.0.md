...
* **스냅샷 기능 전면 개선**
  ...
  * **스냅샷 뷰어 구현**
    ...
    - 개선: 저장된 스크린샷 이미지를 BrowserView에 표시
    - Base64 인코딩으로 data URL 생성하여 오프라인 뷰어 구현
    - **스냅샷 뷰어 UI**
      * 상단 헤더: 제목, 원본 URL 링크, 생성일시
      * 다크 테마 적합 스타일링
      * 이미지 중앙 정렬 및 반응형 크기 조정
      * 원본 URL 클릭 시 새 탭에서 열기
    - 이제 스냅샷을 클릭하면 저장된 스크린샷을 바로 볼 수 있음
