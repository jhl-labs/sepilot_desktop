    ...
    - **Browser 모드 전용 관리 기능**
      ...
      - **Browser 설정** - Browser 전용 설정 다이얼로그
        ...
        - **Browser Agent LLM 설정** (BrowserSettings.tsx)
          ...
          - Max Iterations: 최대 반복 횟수 (1-50, 기본값: 20)
...
- **MarkdownRenderer "URI malformed" 에러 수정**
  - Editor에서 마크다운 파일 미리보기 시 "URI malformed" 에러가 발생하던 문제 수정
  - 문제 원인: 마크다운 내용에 잘못된 형식의 URL이 포함되어 있을 때 `markdown-to-jsx`가 파싱 단계에서 실패
  - 해결 - 3단계 방어 전략:
    1. **마크다운 전처리 (Pre-processing)**
       - `sanitizeMarkdownContent()` 함수로 마크다운 링크 `[text](url)` 패턴 추출
       - URL 유효성 검사 후 문제가 있으면 안전한 URL로 변환
       - `markdown-to-jsx`에 전달하기 전에 미리 정화
    2. **CustomLink 컴포넌트 다층 방어**
       - 상대 경로(`/`, `#`)는 그대로 유지
       - 절대 URL은 `new URL()` → `encodeURI()` → `#invalid-url` 순서로 fallback
       - 일반 URL은 `encodeURI()` → 수동 문자 이스케이프 순서로 처리
       - 렌더링 실패 시 `<span>`으로 대체하여 텍스트만 표시
    3. **렌더링 에러 핸들링**
       - `renderMarkdownContent()` 함수에 try-catch 추가
       - 렌더링 실패 시 에러 메시지 + 원본 내용 표시
  - 이제 어떤 형태의 잘못된 URL이 포함된 마크다운도 안전하게 렌더링됨
