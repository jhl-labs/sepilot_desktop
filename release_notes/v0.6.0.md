  ...
  * **브라우저 기능 추가**
    ...
    - **크롬 스타일 탭 UI**
      ...
      * Settings 드롭다운 메뉴 (MoreVertical 아이콘)
        - 북마크 관리 (예정)
        - 세션 저장 (예정)
        - **Browser Agent 도구 목록**: 12개 built-in tool 목록 표시
          - Navigation (1): browser_navigate
          - Page Inspection (4): get_page_content, get_interactive_elements, get_selected_text, take_screenshot
          - Page Interaction (3): click_element, type_text, scroll
    ...
    - **Browser 모드 전용 관리 기능**
      ...
      * **새 대화** - 브라우저 채팅 내역 초기화
      * **페이지 캡처** - 현재 페이지를 Pocket처럼 스냅샷 형태로 저장 (구현 예정)
      * **스냅샷 관리** - 캡처된 페이지를 그리드 카드로 표시, 클릭 시 로드 (구현 예정)
        - 썸네일, 제목, URL, 생성일시 표시
        - 삭제 기능
      * **북마크 관리** - 즐겨찾기 관리
        - 폴더 기능 (추가, 삭제)
        - 현재 페이지 북마크 추가 (구현 예정)
        - 북마크 클릭 시 BrowserPanel에서 로드 (구현 예정)
