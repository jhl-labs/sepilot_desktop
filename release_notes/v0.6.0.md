...
* **Editor 모드 추가 (Chat + Editor 듀얼 모드)**
  ...
  * **Editor 모드**: 파일 편집 기능
    ...
    - 다크/라이트 테마 자동 적용
  * Monaco Editor CDN 차단 문제 해결:
    - 로컬 파일 사용으로 CSP 정책 준수
    - postinstall 스크립트로 자동 복사
  * 새로운 IPC 핸들러 추가:
...
* **Monaco Editor 로딩 문제 해결 (webpack 플러그인 방식으로 변경)**
  * Monaco Editor가 CDN 또는 public 디렉토리에서 로드되지 않던 문제 수정
  * `monaco-editor-webpack-plugin` 사용으로 완전한 번들링 방식 적용
  * Next.js webpack 설정에 MonacoWebpackPlugin 추가
  * 모든 주요 언어 지원 (javascript, typescript, css, html, json, markdown, python, java, cpp, csharp, go, rust, yaml, xml, sql, shell)
  * 로컬 파일 복사 방식 제거 (더 이상 postinstall 스크립트 불필요)
...
* **도구 실행 로그 UI 개선 (사용자 친화적 표시)**
  * 너무 자세한 도구 실행 로그를 간결하게 개선
  * 변경 전: "🔧 **Using tools:**\n- `file_list`\n✅ **Tool result** (`file_list`)\n```\n(500자 결과)```"
  * 변경 후:
    - 도구 호출: "🔧 file_list, file_read" (한 줄로 간결하게)
    - 성공 시: "✅ file_list" (결과 내용 숨김)
    - 실패 시: "❌ command_execute `npm install`" + 에러 상세 메시지 (최대 10줄)
  * **에러 메시지에 실행된 명령어/파일 경로 표시**
    - command_execute: 실행된 명령어 표시 (예: `npm --version`)
    - file_read/file_write: 파일 경로 표시 (예: `package.json`)
    - grep_search: 검색 패턴 표시 (예: `function.*main`)
  * **에러 메시지 상세 표시 개선** (새로 추가)
    - 첫 줄만 표시하던 것을 최대 10줄 또는 800자까지 표시
    - stderr/stdout의 실제 에러 내용 확인 가능
    - 들여쓰기로 가독성 향상
    - npm install 실패 시 peer dependency 충돌 등 상세 원인 파악 가능
  * Thinking 내용도 300자로 축약하여 표시
  * ASCII 특수문자가 그대로 노출되지 않고 깔끔한 형태로 표시
  * 사용자가 진행 상황을 한눈에 파악하고 에러 원인을 쉽게 파악 가능
