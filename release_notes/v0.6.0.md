...
- **MarkdownRenderer "URI malformed" 에러 수정**
  ...
  - 문제 원인: 마크다운 내용에 잘못된 형식의 URL이 포함되어 있을 때 `markdown-to-jsx`가 파싱 실패
  - 해결:
    - `CustomLink` 컴포넌트에 URL 검증 및 sanitization 로직 추가
    - 상대 경로(`/`, `#`)는 그대로 유지
    - 절대 URL(`http://`, `https://`)은 URL 생성자로 검증
    - 기타 경우는 `encodeURI()`로 안전하게 인코딩
    - 전체 마크다운 렌더링을 `renderMarkdownContent()` 함수로 감싸 try-catch 에러 핸들링
    - 렌더링 실패 시 에러 메시지와 원본 내용을 보여주는 fallback UI 표시
  - 이제 잘못된 URL이 포함된 마크다운도 안전하게 렌더링됨
