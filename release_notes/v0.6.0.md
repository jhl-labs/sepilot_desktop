...
- **이미지 생성 결과가 대화에 표시되지 않는 문제 수정**
  - NanoBanana, ComfyUI에서 생성된 이미지가 텍스트 출력만 표시되고 실제 이미지는 표시되지 않던 문제 해결
  - **근본 원인 1**: `chat-agent.ts`에서 `toolsNode`가 반환한 `generatedImages`를 state에 병합하지 않음
  - **근본 원인 2**: 이미지 생성 완료 후 바로 루프를 종료하여 `generate` 노드가 다시 호출되지 않음
    - 이미지를 포함한 assistant 메시지가 생성되지 않고 데이터베이스에 저장되지 않음
  ...
  - **`lib/langgraph/graphs/chat-agent.ts`: 핵심 수정 사항**
    - toolsNode 실행 후 generatedImages를 state에 병합
    - 이미지 생성 완료 후 한 번 더 generate 노드를 호출하도록 수정
    - `imageGenerationCompleted` 플래그 추가하여 이미지 생성 완료 시 설정
    - 다음 iteration 시작 전 플래그 확인하여 최종 응답 생성 후 종료
  ...
  - 코드 위치:
    ...
    - `lib/langgraph/graphs/chat-agent.ts:21, 25-30, 50-53, 86, 97-103, 280-282, 351-358` - state 병합 및 루프 제어 (핵심)
