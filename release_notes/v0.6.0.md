...
- **RAG 대용량 문서 처리 개선 (2025 Best Practices 적용)**
  - **Parent Document Retrieval 전략 구현**:
    - 원본 문서 전체를 별도 저장하여 컨텍스트 손실 방지
    - 작은 청크로 검색하고 큰 원본 문서로 응답
    - `parentDocId` 메타데이터로 청크-원본 문서 연결 관리
    - 검색 시 자동으로 원본 문서 반환 (`retrievedFrom`, `matchedChunkIndex` 정보 포함)
  - **Contextual Chunk Enhancement 구현**:
    - 각 청크에 문서 전체 컨텍스트 정보 자동 추가
    - 청크가 어느 문서의 몇 번째 부분인지 명시
    - LLM이 청크만으로도 전체 맥락 파악 가능
  - **2025 업계 표준 청킹 전략 적용**:
    - 기본 청크 크기: 1000자 (~250 tokens, 2025 권장 범위)
    - Overlap: 15% (Unstructured.io, Firecrawl 등 2025 best practice)
    - 자동 overlap 보정 (chunkSize 대비 비율 검증)
    - Sentence-boundary 기반 청킹으로 의미 단위 보존
  - **새로운 API 추가**:
    - `VectorDBService.getById()`: ID로 문서 직접 조회
    - `IndexingOptions.storeParentDocument`: 원본 저장 옵션 (기본 true)
    - `IndexingOptions.addDocumentContext`: 컨텍스트 추가 옵션
  - **기술적 근거**:
    - [Unstructured.io: Chunking Best Practices](https://unstructured.io/blog/chunking-for-rag-best-practices)
  ...
  - **기대 효과**:
    - 대용량 문서에서도 전체 컨텍스트 유지
    - 청크 경계로 인한 정보 손실 완전 방지
    - 더 정확하고 완전한 RAG 응답 생성
  - 코드 위치:
    - `lib/vectordb/types.ts:28-52` - DocumentMetadata에 Parent Doc 필드 추가
    - `lib/vectordb/types.ts:145-152` - IndexingOptions 확장
    - `lib/vectordb/indexing.ts:191-244` - Parent Document Retrieval 구현
    - `electron/services/vectordb.ts:204-397` - 검색 로직 개선 (Parent Doc 반환)
    - `electron/services/vectordb.ts:512-532` - getById() 메서드 추가
    - `electron/ipc/handlers/team-docs.ts:180-202` - 2025 best practice 기본값 적용

- **RAG 검색 시스템 대폭 개선 (하이브리드 검색 + 메타데이터 활용)**
  - **메타데이터 기반 필터링 및 부스팅 시스템 도입**:
    - 폴더 경로, 태그, 카테고리, 출처 기반 필터링 지원
    - 제목 매칭 시 30% 점수 부스팅
    - 폴더 경로 매칭 시 20% 점수 부스팅
    - 태그 매칭 시 15% 점수 부스팅
    - 최신 문서 우선순위 옵션 (시간 감쇠 함수 적용)
  - **하이브리드 검색 (Vector + BM25)**:
    - 벡터 유사도 70% + BM25 키워드 검색 30% 혼합
    - 가중치 조절 가능 (hybridAlpha 파라미터)
    - 문서 길이 기반 정보 밀도 평가
  - **개선된 Rerank 노드**:
    - 쿼리 키워드 기반 제목/폴더/태그/컨텐츠 매칭
    - 메타데이터 일치도에 따른 점수 재조정
    - 상위 3개 문서 선별 시 더 정확한 관련성 판단
  - **SearchOptions 인터페이스 추가**:
    - 필터링 옵션: folderPath, tags, category, source
    - 부스팅 옵션: titleBoost, folderPathBoost, tagBoost
    - 하이브리드 검색 옵션: useHybridSearch, hybridAlpha
    - 기타 옵션: includeAllMetadata, recentBoost, recentBoostDecay
  - **강화된 RAG 시스템 프롬프트**:
    - LLM이 문서를 반드시 참조하도록 명시적 지시 추가
    - 문서 정보를 폴더 경로와 함께 구조화하여 제공
    - 답변 작성 규칙을 명확하게 제시
    - 디버깅을 위한 상세 로깅 추가 (문서 개수, 제목, 컨텍스트 길이)
  - **RAG Main Process LLM 초기화 개선**:
    - Main Process에서 RAG 실행 시 LLM 클라이언트 자동 초기화
    - Database에서 설정 로드 및 V2→V1 변환 지원
    - "LLM client is not configured" 에러 완전 해결
  - **검색 성능 향상**:
    - 특수 문서(폴더 등) 자동 제외
    - 메타데이터 기반 조기 필터링으로 불필요한 벡터 계산 감소
    - 폴더 구조가 검색 결과에 직접 반영됨
  - 코드 위치:
    - `lib/vectordb/types.ts:71-94` - SearchOptions 인터페이스 정의
    - `electron/services/vectordb.ts:204-397` - 하이브리드 검색 로직 구현
    - `lib/langgraph/nodes/retrieve.ts:53-64` - Retrieve 노드 개선
    - `lib/langgraph/nodes/retrieve.ts:120-204` - Rerank 노드 개선
    - `electron/ipc/handlers/vectordb.ts:71-102` - IPC 핸들러 업데이트
    - `lib/langgraph/utils/system-message.ts:65-102` - RAG 시스템 프롬프트 강화
    - `lib/langgraph/graphs/rag.ts:13-48` - LLM 초기화 로직 추가
    - `lib/langgraph/graphs/rag.ts:77-90` - RAG 로깅 개선
...
- **GitHub Sync 기능 추가 (Token 기반)**
  ...
  - 여러 데이터 타입별 선택적 동기화 기능:
    ...
    - **문서 동기화**: RAG 문서 (VectorDB 데이터)
    - **AI 페르소나 동기화**: 사용자 정의 AI 페르소나 설정
    - **이미지 동기화**: 생성된 이미지 메타데이터
    - **대화 동기화**: 대화 내역 및 메시지
  - **에러 자동 리포팅 기능**:
    - 프로그램 에러 발생 시 GitHub Issue로 자동 리포트
    - 에러 메시지, 스택 트레이스, 시스템 정보 자동 수집
    - 민감한 정보(API 키, 토큰, 사용자 경로 등) 자동 제거
    - 중복 이슈 자동 감지 및 스킵
    - 사용자 동의 기반 활성화 (Settings에서 on/off)
    - React Error Boundary를 통한 프론트엔드 에러 캡처
    - 전역 에러 핸들러로 미처리 에러 및 Promise rejection 캡처
    - 개발팀에 실시간 피드백 제공으로 프로그램 품질 개선
  ...
  - **보안 고려사항**:
    ...
    - 에러 리포트에서 민감 정보 자동 제거
  - Settings > System > GitHub Sync에서 설정 및 관리

- **ImageGen 통합 이미지 생성 시스템**
  - ComfyUI 단일 제공자에서 다중 제공자 지원으로 확장
  - **NanoBanana (Google Imagen API) 지원 추가**
    - **API Provider 선택 기능**
      - **NanoBananaAPI.ai** (기본값, 권장):
        - 간단한 API key 인증 방식
        - https://nanobananaapi.ai 에서 API key 발급
        - 즉시 사용 가능한 REST API
        - 비동기 작업 처리 (task polling)
      - **Google Vertex AI** (직접 연결):
        - OAuth 2 access token 필요 (복잡한 인증)
        - Google Cloud Project ID 및 Location 설정 필요
        - 고급 사용자 또는 기업 환경에서 사용
    - **모델 선택 기능** (Vertex AI 사용 시):
      - Imagen 3.0 Standard (imagen-3.0-generate-001): 높은 품질, 더 많은 옵션
      - Imagen 3.0 Fast (imagen-3.0-fast-generate-001): 빠른 생성
      - Settings UI에서 모델 선택 드롭다운 제공
    - 다양한 종횡비 지원 (1:1, 16:9, 9:16, 4:3, 3:4)
    - 최대 8개 이미지 동시 생성 가능 (Standard 모델 기준)
    - Negative 프롬프트 및 Seed 설정 지원
    - **Interactive 옵션 선택 기능**
      - Settings에서 "이미지 생성 시 옵션 물어보기" 토글 추가
      - 활성화 시 LLM이 이미지 생성 전에 사용자에게 자동으로 물어봄:
        - 화면 비율 (aspect ratio) 선택
        - 생성할 이미지 개수 선택
      - `generate_image` tool에 `aspectRatio`, `numberOfImages` 파라미터 추가
      - LLM이 사용자 응답을 받은 후 해당 옵션으로 이미지 생성
    - **Credit/Usage 정보 표시 기능**
      - NanoBanana API 응답에서 토큰 사용량 정보 추출
      - 이미지 생성 완료 후 사용량 정보 자동 표시:
        - 생성된 이미지 개수
        - 사용된 토큰 수 (입력/출력 토큰 분리 표시)
      - ChatArea에서 이미지와 함께 사용량 정보 표시
      - LLM도 사용량 정보를 인지하여 응답에 반영 가능
    - **백엔드 구현 완료**
      - NanoBanana API 클라이언트 (lib/imagegen/nanobanana-client.ts)
      - LangGraph tools.ts에서 provider 분기 로직 추가
      - LangGraph generate.ts에서 askOptionsOnGenerate 조건부 system prompt 추가
      - ComfyUI (단일 이미지) 및 NanoBanana (다중 이미지) 모두 지원
      - InputBox에서 두 형식 모두 처리 가능
      - 이미지 생성 진행 상황 실시간 표시
      - IPC를 통한 Main Process에서 API 호출
  - **통합 설정 UI (ImageGenSettingsTab)**
    - 제공자 선택 (ComfyUI / NanoBanana) 라디오 버튼
    - 탭 기반 제공자별 독립 설정
    - 각 제공자별 개별 enable/disable 토글
    - Provider별 연결 테스트 기능
  - **실시간 Provider 선택 기능**
    - 두 provider가 모두 활성화된 경우 ImageGen 드롭다운에 선택 메뉴 표시
    - 대화 중 원하는 provider를 즉시 선택 가능
    - 선택한 provider가 LangGraph에 자동 전달되어 이미지 생성
  - **이미지 생성 전용 모드**
    - 이미지 생성 토글 활성화 시 자동으로 최적 설정 적용:
      - Thinking Mode: Instant로 고정 (빠른 응답)
      - RAG: 비활성화 (문서 검색 불필요)
      - Tools: generate_image만 사용 (다른 도구 비활성화)
    - 토글 비활성화 시 이전 설정으로 자동 복원
    - UI에서 다른 설정 버튼들이 자동으로 비활성화되어 명확한 UX 제공
  - **자동 마이그레이션**
    - 기존 ComfyUI 설정 자동 감지 및 ImageGen 구조로 마이그레이션
    - localStorage 및 DB 양쪽에서 모두 지원
    - 하위 호환성 유지 (comfyUI 필드 병행 지원)
  - Settings > Integrations > ImageGen에서 설정 및 관리
- **AI 페르소나 시스템 추가**
...
- **AI Presentation Lab 추가 (엔터프라이즈 수준 프레젠테이션 생성)**
  - **엔터프라이즈 디자인 시스템 구현**
    - 2025 디자인 트렌드 적용: 미니멀리즘, 그라데이션, 큰 타이포그래피
    - 16:9 비율 슬라이드, 깔끔한 여백과 간격
    - accentColor 기반 일관된 색상 시스템
    - Spectacle, Reveal.js, Slidev 등 오픈소스 참고
  - **5가지 전문 레이아웃 지원**
    - **Hero Layout**: 전체화면 타이틀 슬라이드 (제목 + 부제목 + 이미지)
    - **Title-Body Layout**: 클래식 슬라이드 (제목 + 불릿 포인트 + 이미지)
    - **Two-Column Layout**: 분할 콘텐츠 레이아웃 (좌우 컬럼)
    - **Timeline Layout**: 프로세스/로드맵 시각화 (단계별 번호 표시)
    - **Grid Layout**: 데이터/기능 쇼케이스 (2x2 그리드)
  - **인터랙티브 슬라이드 뷰어**
    - Carousel 방식으로 슬라이드 하나씩 크게 표시
    - 화살표 키 네비게이션 (← → 키로 이동)
    - F키로 전체화면 토글
    - 하단 썸네일 네비게이션 바
    - 좌우 화살표 버튼
    - 현재 슬라이드 번호 표시 (예: "슬라이드 1 / 5")
  - **AI 기반 슬라이드 생성**
    - Sidebar에서 대화식 브리핑 입력
    - ppt-agent가 슬라이드 개요와 이미지 프롬프트 자동 생성
    - LLM 설정 연동 (activeBaseModel, proxy, SSL 설정 적용)
    - 실시간 스트리밍 응답 (Loader 애니메이션, "생성 중..." 표시)
    - JSON 형식으로 구조화된 슬라이드 데이터 (제목, 설명, 불릿, 이미지 등)
    - 고급 설정: Tone, Visual Direction, Color Palette, Typography
    - **언어 자동 감지 시스템 (3단계 우선순위)**
      - 1순위: 명시적 언어 지정 ("영어로", "in English", "日本語で", "中文" 등)
      - 2순위: 사용자 입력 언어 자동 감지 (한글 30%, 영어 50%, 일본어 20%, 중국어 20% 임계값)
      - 3순위: 기본값 (한국어)
      - 슬라이드 콘텐츠와 LLM 응답이 모두 감지된 언어로 생성됨
    - **슬라이드 개수 자동 추출**
      - 사용자 요청에서 슬라이드 개수 자동 인식 ("15페이지", "10 slides", "약 20장" 등)
      - 정확히 요청한 개수만큼 슬라이드 생성 (기본값: 8개)
    - **직접 JSON 생성 방식**
      - Tool 기반 시스템 제거 (134줄 코드 삭제)
      - 첫 번째 요청에서 즉시 슬라이드 JSON 생성
      - 불필요한 질문 단계 제거, 즉시 프레젠테이션 생성 시작
  - **내보내기 기능**
    - PPTX, PDF, HTML 형식 지원
    - 이미지 자동 생성 (ImageGen 연동)
    - 차트/타임라인/테이블 샘플 데이터 렌더링
    - 내보내기 진행 상태 표시
    - 완료 후 "폴더 열기" 버튼으로 바로 접근
  - **UI/UX 개선**
    - Sidebar = Chat 입력, Main = Slide 미리보기 (역할 명확화)
    - Quick Briefs 템플릿 제공 (AI/Edge 전략 브리핑, 피치덱, 워크숍 아젠다)
    - 중지 버튼으로 생성 취소 가능
    - 슬라이드 수동 추가 기능
  - 코드 위치:
    - `components/presentation/SlideRenderer.tsx` - 슬라이드 레이아웃 렌더링
    - `components/presentation/SlidePreview.tsx` - 인터랙티브 뷰어
    - `components/presentation/PresentationChat.tsx` - AI 대화 인터페이스
    - `components/presentation/PresentationStudio.tsx` - 메인 컨테이너
    - `lib/presentation/ppt-agent.ts` - AI 슬라이드 생성 로직
    - `types/presentation.ts` - 타입 정의
...
- **Semantic Chunking 구현 (문서 청킹 전략 개선)**
  - **다양한 청킹 전략 추가**
    - **Sentence-based (권장)**: 문장 경계를 보존하여 청킹, 컨텍스트 손실 최소화
    - **Structure-aware**: Markdown 헤딩, 코드 블록 등 문서 구조 보존 (기술 문서에 적합)
    - **Character-based (기존)**: 단순 문자 수 기반 분할 (빠르지만 문맥 손실 가능)
    - **Token-based (향후 지원)**: LLM 토큰 단위 청킹 (현재 sentence로 폴백)
  - **기존 문제점 해결**
    - 기존 character-based는 단어/문장 중간에서 자르는 문제로 검색 품질 저하
    - 예: "서울특별시"가 "서울특", "별시"로 분리되어 검색 실패
  - **Sentence-based 청킹 알고리즘**
    - 문장 경계 정규식으로 ., !, ?, \n 등을 인식 (한글/영어 모두 지원)
    - 문장 단위로 청크를 구성하며 chunkSize 초과 시 청크 분리
    - Overlap은 마지막 N개 문장을 유지하여 문맥 보존
  - **Structure-aware 청킹 알고리즘**
    - Markdown 헤딩 (# ~ ######), 코드 블록 (```), 리스트 (-, \*, +, 숫자.) 인식
    - 구조적 섹션을 기준으로 청크 분리하여 의미 단위 유지
    - 구조가 없는 경우 sentence-based로 자동 폴백
  - **UI 개선**
    - DocumentUploadDialog에 청킹 전략 선택 드롭다운 추가
    - 각 전략별 설명 툴팁 제공
    - 선택한 전략이 metadata에 저장되어 추적 가능
  - **구현 파일**
    - lib/vectordb/indexing.ts: chunkTextSentence, chunkTextStructure 함수 추가
    - lib/vectordb/types.ts: ChunkStrategy 타입 및 IndexingOptions에 chunkStrategy 필드 추가
    - components/rag/DocumentUploadDialog.tsx: 청킹 전략 선택 UI
  - **기본값 변경**
    - 기존 character → sentence로 변경 (더 나은 검색 품질)

- **빈 폴더 VectorDB 마이그레이션**
  - **기존 문제점**
    - 빈 폴더가 localStorage에 저장되어 Export/Import 시 손실됨
    - 환경 간 문서 동기화 시 폴더 구조가 유지되지 않음
  - **VectorDB 통합 저장**
    - 빈 폴더를 특수 문서 타입(`_docType: 'folder'`)으로 VectorDB에 저장
    - 폴더 문서는 빈 content와 0으로 채워진 embedding으로 저장되어 검색에서 제외됨
    - Export/Import 시 폴더 구조가 자동으로 포함됨
  - **API 추가**
    - lib/vectordb/client.ts: createEmptyFolder, deleteEmptyFolder, getAllEmptyFolders 함수 추가
    - electron/ipc/handlers/vectordb.ts: 폴더 관련 IPC 핸들러 추가
    - electron/preload.ts: 폴더 관리 메서드 추가
    - types/electron.d.ts: VectorDBAPI에 폴더 관련 타입 추가
  - **UI 업데이트**
    - DocumentList: localStorage 대신 VectorDB 사용하도록 수정
    - 폴더 생성/삭제 시 자동으로 VectorDB에 반영
    - 문서 로드 시 폴더 문서 자동 필터링 (\_docType !== 'folder')
  - **마이그레이션 전략**
    - 기존 localStorage의 빈 폴더는 자동 마이그레이션 안 함 (수동 재생성 필요)
    - 새로 생성되는 모든 폴더는 VectorDB에 저장됨
...
- **이미지 생성 결과가 대화에 표시되지 않는 문제 수정 (최종 해결)**
  - NanoBanana, ComfyUI에서 생성된 이미지가 텍스트 출력만 표시되고 실제 이미지는 표시되지 않던 문제 완전 해결
  - **근본 원인 1**: `chat-agent.ts`에서 `toolsNode`가 반환한 `generatedImages`를 state에 병합하지 않음
  - **근본 원인 2**: 이미지 생성 완료 후 iteration 2 시작 시점에서 루프를 조기 종료하여 `generate` 노드가 호출되지 않음
    - iteration 2가 시작되자마자 "이미지 생성 완료" 체크가 루프를 종료시켜 이미지를 포함한 assistant 메시지가 생성되지 않음
    - 결과적으로 이미지가 데이터베이스에 저장되지 않아 UI에 표시되지 않음
  - **근본 원인 3**: `useMessageStreaming` 훅이 `tools` 노드의 `toolResults`에서만 이미지를 추출
    - 수정된 구조에서는 `generate` 노드의 메시지에 이미지가 첨부됨
    - `generate` 노드 처리 시 메시지의 이미지를 확인하지 않아 UI 업데이트 안됨
  - `AgentState`, `CodingAgentState`에 `generatedImages` 필드 추가하여 생성된 이미지를 State에 저장
  - `lib/langgraph/nodes/tools.ts`: 이미지 생성 도구 실행 시 생성된 이미지 객체를 `generatedImages` 필드로 반환하도록 수정
  - `lib/langgraph/nodes/generate.ts`: Assistant 메시지 생성 시 State의 `generatedImages`를 메시지에 첨부하도록 수정
  - **`lib/langgraph/graphs/chat-agent.ts`: 루프 제어 수정 (핵심 1)**
    - toolsNode 실행 후 generatedImages를 state에 병합
    - generateNode 실행 후 generatedImages와 toolResults를 state에 병합
    - **조기 루프 종료 체크 제거**: iteration 시작 시점의 `imageGenerationCompleted` 체크를 제거
    - 이제 이미지 생성 후 자연스럽게 iteration 2가 실행되어 generate 노드가 이미지를 포함한 최종 메시지 생성
    - `shouldUseTool` decision 로직이 자연스럽게 루프를 종료시킴 (tool_calls가 없으므로)
    - `invoke()` 및 `stream()` 메서드 모두 수정
  - **`components/chat/unified/hooks/useMessageStreaming.ts`: UI 이미지 추출 수정 (핵심 2)**
    - `generate` 노드 처리 시 `lastMessage.images` 확인 추가
    - `referenced_documents`와 함께 `images`를 assistant 메시지에 업데이트
    - 기존 `tools` 노드 `toolResults` 방식과 함께 동작하도록 수정
  - `lib/langgraph/state.ts`: `createInitialAgentState`, `createInitialCodingAgentState` 함수에 `generatedImages: []` 추가
  - 모든 thinking mode graph 파일 (`deep-thinking.ts`, `sequential-thinking.ts`, `tree-of-thought.ts`) state 초기화 시 `generatedImages: []` 추가
  - 생성된 이미지가 이제 대화 화면과 Gallery에서 정상적으로 표시됨
  - 코드 위치:
    - `lib/langgraph/state.ts:76-88, 123-135` - generatedImages 필드 정의
    - `lib/langgraph/nodes/tools.ts` - 이미지 생성 도구 수정 (NanoBanana, ComfyUI)
    - `lib/langgraph/nodes/generate.ts:447-450` - Assistant 메시지에 이미지 첨부
    - `lib/langgraph/graphs/chat-agent.ts:23-34, 44-54` - 조기 종료 체크 제거 및 state 병합 수정 (핵심 1)
    - `components/chat/unified/hooks/useMessageStreaming.ts:321-346` - generate 노드 이미지 추출 추가 (핵심 2)
    - `types/index.ts:10-17` - ImageAttachment 타입 수정 (path 필드 optional)
...
- **Browser Agent 에러 복구 전략 강화 (Phase 2)**
  ...
  - **CRITICAL 규칙 추가: 실패한 도구를 같은 인자로 반복 호출 금지**
    - "같은 도구 + 같은 인자"로 재시도하면 똑같이 실패함을 명시
    - 최대 1회 재시도 규칙 적용 (예: google_visit_result rank=5 실패 시 다시 rank=5 시도 금지)
    - 타임아웃 늘리기보다 대안 도구 사용 권장 (maxWaitTime 기본값 10초 유지)
  - **google_visit_result 파라미터 가이드 추가**
    - maxWaitTime 기본값(10초) 사용 권장, 늘리지 말 것
    - 실패 시 browser_navigate로 즉시 전환 유도
  - **google_visit_result 페이지 로딩 개선**
    - `did-finish-load` → `dom-ready` 이벤트로 변경 (더 빠르고 안정적)
    - `did-fail-load` 이벤트 핸들러 추가로 로딩 실패 즉시 감지
    - JavaScript 대기 시간 2000ms → 1500ms 단축
    - 에러 메시지에 "try browser_navigate instead" 힌트 포함
  - LLM의 도구 호출 오류 및 반복 실패 패턴 대폭 감소
...
- **Browser Agent 타입 정의 대폭 개선 (Perplexity Comet / OpenAI Operator 수준)**
  - **파일 구조 개선**: types/browser-agent.ts → types/browser-agent/ 디렉토리로 분리
    - `google-search.ts`: Google Search Tools (기존 9개)
  ...
  - **Browser Control Tools 14개 타입 정의 추가** (OpenAI Operator 수준)
    - Navigation (1개): `browser_navigate`
    - Inspection (5개): `browser_get_page_content`, `browser_get_interactive_elements`, `browser_search_elements`, `browser_get_selected_text`, `browser_take_screenshot`
    - Interaction (4개): `browser_click_element`, `browser_type_text`, `browser_scroll`, `browser_wait_for_element`
    - Tabs (4개): `browser_list_tabs`, `browser_create_tab`, `browser_switch_tab`, `browser_close_tab`
  - **Vision Tools 5개 타입 정의 추가** (Set-of-Mark, Coordinate Clicking)
    - `browser_capture_annotated_screenshot`: Set-of-Mark (SoM) 방식 스크린샷
    - `browser_click_coordinate`: 정확한 픽셀 좌표 클릭
    - `browser_click_marker`: 마커 라벨(A, B, C...)로 클릭
    - `browser_get_clickable_coordinate`: Element ID → 클릭 가능한 좌표 변환
    - `browser_analyze_with_vision`: Vision LLM 페이지 분석
  - **통합 에러 시스템**
    - 30개 BrowserErrorType 정의 (navigation, element, interaction, page, tab, screenshot, search, network)
    - BrowserError 인터페이스 (타입, 메시지, 컨텍스트, recoverable, suggestedAction)
    - ErrorRecoveryStrategy (재시도 로직, fallback actions)
    - 6개 사전 정의 복구 전략 (navigation_failed, element_not_found, element_not_clickable, search_timeout, page_crash, screenshot_failed)
  - **Workflow & Session Management**
    - BrowserSession (goal, planned steps, progress, visited pages, tool calls, errors)
    - BrowserWorkflowStep (type, description, tool, arguments, status, retry count)
    - BrowserSessionAnalytics (duration, tool performance, error recovery rate)
    - BrowserWorkflowPlan, BrowserMultiStepTask
  - **핵심 반환 타입 정의**
    - PageContent (semantic structure, headings, sections, interactive element counts)
    - InteractiveElement (Accessibility Tree 기반, role, label, position, context)
    - AnnotatedScreenshotResult (markers, bounding boxes, confidence scores)
    - BrowserTab (id, title, url, loading state)
    - ElementSearchResult (natural language search results with relevance scores)
  - **통합 Tool 목록**
    - ALL_BROWSER_AGENT_TOOLS: 28개 도구 통합 (Google Search 9개 + Browser Control 14개 + Vision 5개)
    - BROWSER_AGENT_TOOLS_BY_CATEGORY: 7개 카테고리별 분류 (search, extraction, navigation, inspection, interaction, tabs, vision)
    - BROWSER_AGENT_TOOL_COUNTS: 카테고리별 도구 개수
  - **하위 호환성 유지**: 기존 types/browser-agent.ts는 types/browser-agent/index.ts로 리다이렉션
  - **TypeScript strict mode 완벽 지원**: 모든 타입 에러 해결
...
- **GitHub 통합 기능 문서화 개선**
  - **DocumentUploadDialog에 GitHub 기능 안내 섹션 추가**
    - **재귀적 디렉토리 탐색**:
      - 디렉토리 경로 입력 시 모든 하위 폴더를 재귀적으로 탐색
      - .md, .txt, .json, .yaml, .yml 파일만 자동 수집
      - 단일 파일 경로 입력 시 해당 파일만 가져옴
    - **Private Repository 지원**:
      - GitHub Personal Access Token 입력 필드 제공
      - Token 생성 방법 안내 (Settings > Developer settings > Personal access tokens)
      - 필수 권한 안내 (repo 또는 public_repo scope)
      - Public repository는 token 없이도 접근 가능
    - 사용자 편의성 개선으로 GitHub 기능 활용도 향상

- **RAG 문서 검색 및 필터링 기능 추가**
  - **DocumentList에 검색창 추가**
    - 제목, 폴더 경로, 내용 실시간 검색
    - 청크된 문서의 모든 내용까지 검색
    - 대소문자 구분 없는 검색 (case-insensitive)
  - **검색 UI**:
    - 상단에 Search 아이콘이 있는 검색 입력창
    - X 버튼으로 검색어 즉시 초기화
    - 검색 결과 개수 표시 (예: "5 / 100개")
    - "검색 결과가 없습니다" 안내 메시지
  - **필터링 로직**:
    - filterDocuments() 함수로 모든 뷰 모드에서 작동
    - Grid, List, Tree 뷰 모두 검색 결과 반영
    - 빈 폴더 제외한 문서만 검색 대상
  - **사용자 경험 개선**:
    - 대량 문서에서 원하는 문서 빠르게 찾기
    - 특정 주제/키워드 관련 문서만 표시
    - 폴더 이름으로 필터링 가능

### 보안

- **VectorDB SQL Injection 취약점 수정**
  - **문제**: 인덱스/테이블 이름이 파라미터화 없이 직접 SQL 쿼리에 삽입되어 SQL Injection 공격에 취약
  - **validateIndexName() 메서드 추가**
    - 정규식 검증: 영문자, 숫자, 언더스코어만 허용 (첫 글자는 영문자 또는 언더스코어)
    - SQL 예약어 체크: SELECT, INSERT, UPDATE, DELETE, DROP, CREATE, TABLE 등 금지
    - 잘못된 입력 시 명확한 에러 메시지 제공
  - **적용 위치**:
    - initialize(): 초기화 시 인덱스 이름 검증
    - createIndex(): 테이블 생성 전 검증
    - deleteIndex(): 테이블 삭제 전 검증
  - **보안 강화**: 악의적인 인덱스 이름으로 인한 데이터베이스 조작 차단

- **GitHub API Rate Limit 처리 개선**
  - **문제**: GitHub API Rate Limit (60 req/hour, 인증 시 5000 req/hour) 초과 시 요청 실패
  - **fetchWithRetry() 함수 추가**
    - 최대 3회 재시도 로직 (configurable)
    - 429 (Too Many Requests) 응답 시 Retry-After 헤더 확인
    - 지수 백오프 알고리즘 (1s → 2s → 4s, 랜덤 jitter 추가)
    - Rate Limit 헤더 감시 (x-ratelimit-remaining, x-ratelimit-reset)
  - **자동 대기 및 재시도**:
    - Rate Limit 소진 시 reset 시점까지 자동 대기
    - 네트워크 오류 시 지수 백오프로 재시도
    - 모든 GitHub API 요청에 적용 (fetchGitHubDocument, fetchGitHubDirectory)
  - **사용자 경험 개선**:
    - 대량 문서 업로드 시 안정성 향상
    - 명확한 에러 메시지 및 로그 (콘솔에 대기 시간 표시)
    - Private repo 사용 시 더 높은 Rate Limit 안내
...
- **VectorDB 검색 성능 최적화 (O(n) → Top-K 알고리즘)**
  - **문제**: 모든 문서를 메모리에 로드하고 정렬하여 대량 문서에서 성능 저하
  - **최적화 전략**:
    - **소량 문서(k\*2 이하)**: 전체 정렬 알고리즘 사용 (단순하고 빠름)
    - **대량 문서**: Min-Heap 기반 Top-K 추출 알고리즘 적용
  - **Min-Heap 알고리즘 상세**:
    - k개 결과만 메모리에 유지 (메모리 효율성)
    - 최소 점수(minScore) 캐싱으로 조기 종료 최적화
    - 삽입 정렬로 부분 정렬 유지 (O(k) 정렬 비용)
    - 점수가 minScore 이하인 문서는 스킵 (불필요한 JSON 파싱 방지)
  - **성능 개선**:
    - 1000개 문서: O(1000) → O(k \* 1000) (k는 보통 3~10)
    - 메모리 사용량: O(n) → O(k)
    - JSON 파싱: n번 → 최대 n번 (조기 종료 시 더 적음)
  - **검증**: 대용량 RAG 문서 컬렉션에서 검색 속도 대폭 향상
...
### AI Presentation Lab 버그 수정

- **SlideRenderer undefined 에러 수정**
  - 슬라이드 로딩 중 `Cannot read properties of undefined (reading 'accentColor')` 에러 발생 문제 해결
  - **문제 원인**: SlideRenderer 컴포넌트가 undefined slide를 받을 수 있는 경우를 처리하지 않음
  - **해결**:
    - slide prop 타입을 `PresentationSlide | undefined`로 변경
    - Early return 패턴으로 undefined 체크 추가
    - 에러 대신 사용자 친화적 메시지 표시: "슬라이드를 불러올 수 없습니다."
  - 코드 위치: `components/presentation/SlideRenderer.tsx:13-21`
...
- **Phase 1: 기본 인프라 구현**
  - Health Check 서비스: Database, VectorDB, MCP, LLM Provider 상태 실시간 검증
  - Test Runner IPC 핸들러: Frontend에서 테스트 실행 및 결과 조회
  - F10 메뉴에 "Test" 항목 추가
    - Run All Tests: 전체 테스트 실행
    - Health Check: 시스템 상태 검증
    - LLM Interaction Tests: LLM Provider 테스트
    - Database Tests: DB/VectorDB 테스트
    - MCP Tool Tests: MCP 서버 및 도구 테스트
    - Open Test Dashboard: 테스트 대시보드 열기 (Phase 3 예정)
  - TypeScript 타입 정의: HealthCheckResult, TestSuiteResult, TestRunnerAPI
  - `window.electronAPI.testRunner` API 노출

- **Phase 2: 핵심 테스트 스위트 구현**
  - **LLM Interaction Tests** (4개 테스트)
    - Provider 설정 확인
    - API 키 검증
    - Token 제한 유효성 검사
    - Temperature 설정 범위 검증
  - **Database & VectorDB Tests** (7개 테스트)
    - SQLite 데이터베이스 연결 확인
    - 읽기/쓰기 작업 검증
    - Conversation 테이블 무결성
    - Message 테이블 무결성
    - VectorDB 연결 상태
    - VectorDB 컬렉션 관리
  - **MCP Tools Tests** (4개 테스트)
    - MCP Manager 초기화 확인
    - MCP 서버 연결 상태
    - 도구 가용성 검증
    - Built-in 도구 확인
...
- **주요 기능**
  - 각 테스트는 독립적으로 실행 가능
  - 상세한 테스트 결과: pass/fail/skip 상태, 소요 시간, 메시지, 에러 정보
  - Health Check 주기적 실행 지원 (기본 60초 간격)
  - 전체 테스트 실행 시 모든 스위트 통합 실행
  - 개발자와 사용자 모두 시스템 상태를 스스로 진단 가능

- **기술적 배경**
  - 2025년 표준 테스트 패턴 적용:
    - Health Check Pattern (Liveness, Readiness, Startup)
    - Mock-Based Testing for 비결정적 LLM 호출
    - Self-Testing Application Pattern
  - Playwright 기반 E2E 테스트 인프라 활용
  - IPC를 통한 Main Process 테스트 실행

**변경된 파일**:

- `electron/services/health-check.ts`: Health Check 서비스 구현
- `electron/ipc/handlers/test-runner.ts`: Test Runner IPC 핸들러
- `electron/tests/suites/llm-interaction.ts`: LLM 상호작용 테스트 스위트
- `electron/tests/suites/database.ts`: Database 테스트 스위트
- `electron/tests/suites/mcp-tools.ts`: MCP Tools 테스트 스위트
- `electron/main.ts`: F10 메뉴에 Test 항목 추가
- `electron/preload.ts`: testRunner API 노출
- `types/electron.d.ts`: TestRunnerAPI 타입 정의

**영향**:

- 매번 수동 테스트 대신 자동화된 검증으로 개발 생산성 향상
- 사용자가 문제 발생 시 스스로 진단 가능
- CI/CD 파이프라인 구축 기반 마련
- 배포 전 전체 기능 검증으로 품질 보증
- LLM Provider 변경 시 안전성 확보

- **Phase 3: Test Dashboard UI 구현**
  - `/test-dashboard` 페이지 (F10 > Test > Open Test Dashboard)
  - **TestDashboard 컴포넌트**
    - Health Check 실시간 표시 및 자동 새로고침
    - 전체/개별 테스트 스위트 실행 버튼
    - Auto-refresh 토글 (60초 간격)
  - **HealthStatus 컴포넌트**
    - Overall system status (healthy/degraded/unhealthy)
    - Database, VectorDB, MCP Tools, LLM Providers 개별 상태 카드
    - Latency 및 상세 정보 표시
  - **TestResultViewer 컴포넌트**
    - 테스트 Summary (total/passed/failed/skipped)
    - Collapsible 카드로 개별 테스트 상세 보기
    - 에러 정보 및 duration 표시
  - **테스트 히스토리**
    - localStorage에 테스트 결과 자동 저장
    - 페이지 재방문 시 마지막 결과 복원
  - **메뉴 통합**
    - F10 메뉴에서 테스트 실행 시 Dashboard에 자동 반영
    - 메뉴 > Open Test Dashboard로 전용 페이지 열기

**변경된 파일**:

- `app/test-dashboard/page.tsx`: Test Dashboard 페이지
- `components/test-runner/TestDashboard.tsx`: 메인 대시보드 컴포넌트
- `components/test-runner/HealthStatus.tsx`: Health Check 결과 시각화
- `components/test-runner/TestResultViewer.tsx`: 테스트 결과 상세 표시
- `app/page.tsx`: IPC 이벤트 리스너 추가 (Dashboard 열기)

**영향**:

- 시각적으로 테스트 결과를 확인 가능
- 개발자와 사용자 모두 직관적인 UI로 시스템 상태 파악
- 테스트 히스토리를 통해 변화 추적 가능
- shadcn/ui 컴포넌트로 일관된 디자인 유지

### 버그 수정

- **Test Dashboard 버튼 클릭 무응답 문제 수정**
  - 문제: LLM Interaction Test, Database Test, MCP Test 버튼 클릭 시 아무 반응이 없었음
  - 원인: 테스트 실행 함수들이 useEffect 이후에 정의되어 함수 호이스팅/타이밍 문제 발생
  - 해결 방법:
    - 모든 테스트 실행 함수를 useEffect 이전에 정의
    - React.useCallback으로 함수 메모이제이션하여 안정적인 참조 보장
    - useEffect 의존성 배열에 콜백 함수 추가
  - 영향: Test Dashboard의 모든 버튼이 정상적으로 동작하게 됨
  - 수정된 파일: `components/test-runner/TestDashboard.tsx`

- **LLM 테스트 및 Health Check의 데이터베이스 접근 패턴 수정**
  - 문제:
    - LLM 설정이 정상적으로 있음에도 테스트가 계속 실패
    - `"undefined" is not valid JSON` 에러 발생
    - configResult.value가 undefined로 나옴
  - 원인:
    - `db.prepare().get()`을 사용하면 SQL.js의 결과 포맷이 예상과 다름
    - `db.exec()`과 `db.prepare().get()`의 반환 형식 차이
    - 직접 SQL 쿼리 사용으로 인한 불일치
  - 해결 방법:
    - 모든 직접 SQL 쿼리를 `databaseService.getSetting()`으로 대체
    - `getSetting()`은 `db.exec()` 결과 포맷을 올바르게 처리
    - llm-interaction.ts의 4개 테스트 모두 수정 (testProviderConfiguration, testAPIKeyValidation, testTokenLimits, testTemperatureSettings)
    - health-check.ts의 checkLLMProviders() 수정
  - 영향:
    - LLM 테스트가 이제 정상적으로 설정을 읽어서 검증
    - Health Check도 올바른 설정 정보 표시
    - 데이터베이스 접근 일관성 확보
  - 수정된 파일:
    - `electron/tests/suites/llm-interaction.ts`
    - `electron/services/health-check.ts`

- **각 대화마다 개별 UnifiedChatInput 설정 저장 및 복원**
  - 문제: 모든 대화가 동일한 전역 설정을 공유하여 대화 전환 시 설정이 유지되지 않음
  - 해결:
    - 각 대화(Conversation)가 고유한 채팅 설정(chatSettings)을 가지도록 구현
    - 대화 생성 시 현재 설정이 해당 대화에 저장됨
    - 대화 전환 시 저장된 설정이 자동으로 복원됨
  - 주요 변경사항:
    - `ConversationChatSettings` 인터페이스 추가: thinkingMode, enableRAG, enableTools, enableImageGeneration 등 저장
    - Database 스키마 확장: conversations 테이블에 chatSettings 컬럼 추가 (JSON 형식)
    - chat-store 업데이트: 모든 설정 변경 함수가 현재 대화 설정도 자동 업데이트
  - 영향:
    - 대화별로 독립적인 설정 관리 가능
    - 작업 컨텍스트 전환 시 설정 재조정 불필요
    - 사용자 경험 크게 개선
  - 수정된 파일:
    - `types/index.ts`: ConversationChatSettings 타입 정의
    - `lib/store/chat-store.ts`: 설정 저장/복원 로직
    - `electron/services/database.ts`: DB 스키마 및 쿼리 수정

- **이미지 생성 활성화 시 Tools 비활성화 방지**
  - 문제:
    - 이미지 생성(enableImageGeneration)이 활성화된 상태에서 Tools를 비활성화하면 이미지 생성이 실패함
    - 이전 대화 복원 시 enableImageGeneration=true, enableTools=false 상태가 되어 Tools 토글이 비활성화되고 변경 불가능
  - 원인:
    - generate_image는 MCP tool로 구현되어 enableTools가 필수
    - UI에서 Tools 버튼이 disabled 되어 사용자가 켤 수도 없는 데드락 상태
  - 해결:
    - `setEnableTools`에서 이미지 생성이 활성화된 경우 Tools 비활성화 차단
    - `setActiveConversation`에서 대화 복원 시 일관성 검증:
      - enableImageGeneration=true이면 enableTools=true 강제 설정
      - enableRAG=false, thinkingMode='instant' 강제 설정
    - UnifiedChatInput에서 Tools 버튼의 disabled 조건 수정
  - 영향:
    - 이미지 생성 기능이 안정적으로 동작
    - 기존 대화 복원 시에도 일관성 보장
    - 사용자가 불가능한 상태에 빠지지 않음
  - 수정된 파일:
    - `lib/store/chat-store.ts`: setEnableTools, setActiveConversation 수정
    - `components/chat/unified/UnifiedChatInput.tsx`: disabled 조건 수정

- **토글 아이콘 간격 개선**
  - 문제: UnifiedChatInput의 토글 버튼들 간격이 좁아서 토글 시 UI가 어색하게 보임
  - 해결: 모든 레이아웃 모드(ultra-compact, compact, full)에서 gap-1을 gap-2로 변경
  - 영향: 토글 버튼들 간의 여백이 증가하여 가독성 및 UX 향상
  - 수정된 파일: `components/chat/unified/UnifiedChatInput.tsx`

- **RAG 검색 고도화: SearchOptions 및 BM25 하이브리드 검색 구현**
  - **SearchOptions 완전 구현**: VectorDB 검색 시 다양한 필터링 및 부스팅 옵션 제공
    - 필터링 옵션: folderPath, tags, category, source로 문서 사전 필터링
    - 메타데이터 부스팅: 제목, 폴더 경로, 태그 매칭 시 점수 상승
    - 최신성 부스팅: 최근 문서에 더 높은 가중치 부여 (지수 감쇠 방식)
  - **BM25 하이브리드 검색**: 벡터 유사도와 키워드 기반 BM25 점수 결합
    - 벡터 검색 (코사인 유사도)과 키워드 검색 (BM25) 점수를 alpha 가중치로 조합
    - BM25 알고리즘 완전 구현: TF (Term Frequency), 문서 길이 정규화 (k1=1.5, b=0.75)
    - 한글 지원 토큰화: 영문, 숫자, 한글을 모두 올바르게 분리 처리
  - **키워드 매칭 개선**: 제목/내용에 쿼리 키워드 포함 시 부스팅 점수 증가
  - **queryText 파라미터 추가**: IPC 핸들러 → preload → TypeScript 타입 정의까지 전체 파이프라인 업데이트
    - `electron/ipc/handlers/vectordb.ts`: search 핸들러에 queryText 파라미터 추가
    - `electron/preload.ts`: vectorDB.search 메서드에 queryText 전달
    - `types/electron.d.ts`: VectorSearchOptions 타입 정의 및 export
  - **기술적 배경**:
    - 기존 벡터 검색만으로는 정확한 키워드 매칭 어려움
    - 의미적 유사도(벡터)와 어휘적 유사도(BM25)를 결합하여 검색 품질 향상
    - SearchOptions의 다양한 부스팅 옵션으로 도메인별 최적 검색 가능

**변경된 파일**:

- `electron/services/vectordb.ts`:
  - searchByVector()에 queryText 파라미터 추가
  - calculateBM25Score() 메서드 구현 (BM25 알고리즘)
  - tokenize() 메서드 구현 (한글 지원 토큰화)
  - containsKeywords() 메서드 수정 (키워드 매칭)
- `electron/ipc/handlers/vectordb.ts`: search 핸들러 시그니처 업데이트
- `electron/preload.ts`: vectorDB.search 시그니처 업데이트
- `types/electron.d.ts`: VectorSearchOptions 인터페이스 추가 및 export

**영향**:

- RAG 문서 검색 정확도 대폭 향상 (벡터 + 키워드 결합)
- 폴더/태그 기반 필터링으로 대규모 문서에서도 빠른 검색
- 최신 문서 우선 검색으로 시간에 민감한 정보 처리 개선
- 제목 키워드 매칭으로 관련성 높은 문서 우선 노출
- 사용자가 SearchOptions를 통해 검색 전략 세밀하게 조정 가능

### Team Docs 협업 기능 구현 (Push/Pull 워크플로우)

- **GitHub에 문서 Push 기능 추가**: Team Docs 편집 후 GitHub에 업로드 가능
  - `GitHubSyncClient.pushDocument()` 메서드 구현
  - Markdown 형식으로 자동 변환하여 GitHub에 커밋
  - 충돌 감지: SHA 불일치 시 `CONFLICT` 에러 반환
- **문서 버전 추적 메타데이터 추가**:
  - `githubSha`: GitHub 파일의 SHA (버전 추적용)
  - `githubPath`: GitHub 경로 (예: sepilot/documents/folder/doc.md)
  - `lastPulledAt`: 마지막 Pull 시간
  - `modifiedLocally`: 로컬 편집 여부 플래그
  - `lastPushedAt`: 마지막 Push 시간

- **DocumentEditDialog에 "저장 & Push" 버튼 추가**:
  - Team Docs 편집 시 "로컬 저장" / "저장 & Push" 두 옵션 제공
  - Personal Docs는 기존과 동일하게 "저장" 버튼만 표시
  - Push 시 충돌 감지하여 사용자에게 알림

- **DocumentList에 로컬 수정 상태 표시**:
  - Team Docs가 로컬에서 수정된 경우 "로컬 수정됨 (Push 필요)" 배지 표시
  - 시각적으로 Push가 필요한 문서 구분 가능

- **Personal Docs와 Team Docs 명확한 구분**:
  - DocumentUploadDialog의 모든 업로드(수동, HTTP, GitHub)에 `docGroup='personal'` 명시적 설정
  - Team Docs는 오직 Team Docs 동기화를 통해서만 생성 (`docGroup='team'`)

- **IPC 핸들러 및 타입 정의**:
  - `team-docs-push-document` IPC 핸들러 추가
  - Push 성공 시 로컬 VectorDB 메타데이터 자동 업데이트 (githubSha, modifiedLocally)
  - `TeamDocsAPI.pushDocument()` TypeScript 타입 정의

**기술적 배경**:

- **Three-Way Merge 준비**: githubSha를 통해 Base, Local, Remote 비교 가능
- **충돌 감지**: GitHub API의 SHA 검증으로 동시 편집 충돌 감지
- **협업 워크플로우**: Pull (동기화) → Edit (편집) → Push (업로드) 사이클 구현

**변경된 파일**:

- `lib/github/client.ts`: pushDocument(), formatDocumentAsMarkdown() 메서드 추가
- `electron/ipc/handlers/team-docs.ts`: team-docs-push-document 핸들러 추가
- `electron/preload.ts`: teamDocs.pushDocument API 추가
- `types/electron.d.ts`: TeamDocsAPI.pushDocument 타입 정의
- `components/rag/DocumentEditDialog.tsx`: Save & Push 버튼 추가, 충돌 처리 로직
- `components/rag/DocumentList.tsx`: modifiedLocally 상태 배지 표시
- `components/rag/DocumentUploadDialog.tsx`: docGroup='personal' 명시적 설정

**영향**:

- **진정한 협업 가능**: 여러 사용자가 Team Docs를 편집하고 GitHub에 반영 가능
- **데이터 손실 방지**: 로컬 편집 플래그로 동기화 시 덮어쓰기 방지
- **충돌 감지**: SHA 기반 충돌 감지로 동시 편집 문제 해결
- **투명한 상태**: UI에서 Push 필요 여부를 명확히 표시
- **단계적 개선 기반**: 향후 Three-Way Merge, Auto-Pull 등 고급 기능 추가 가능

**주의사항**:

- Push 전에 최신 버전 Pull 권장 (충돌 최소화)
- 충돌 발생 시 먼저 동기화 후 재편집 필요
- Merge Conflict 수동 해결 UI는 향후 추가 예정 (P1)

- **우측 하단 도구 아이콘 디자인 개선**
  - 문제: 드롭다운 버튼에 ChevronDown 아이콘이 있어 아이콘이 중앙에 배치되지 않고 시각적으로 어색함
  - 해결:
    - Thinking Mode, Tools, Image Generation 드롭다운 버튼에서 ChevronDown 제거
    - border-2 border-muted-foreground/30 추가로 드릴다운 메뉴임을 표시
    - 아이콘이 정 가운데 배치되어 시각적으로 균형잡힌 디자인
  - 영향: 더 깔끔하고 직관적인 UI, 드릴다운 기능은 border로 인지 가능
  - 수정된 파일: `components/chat/unified/UnifiedChatInput.tsx`

- **Quick Input/Question 자동 전송 기능 추가**
  - 문제: Quick Input과 Quick Question에서 메시지 입력 후 Enter 또는 Send 버튼을 눌러야만 전송됨
  - 해결:
    - Quick Input과 Quick Question에서 메시지가 입력되면 자동으로 새 대화를 생성하고 LLM 요청까지 즉시 실행
    - 사용자가 Enter나 Send 버튼을 누를 필요 없이 Spotlight처럼 빠르게 질문 가능
  - 구현:
    - `UnifiedChatInput.tsx`에 `sepilot:auto-send-message` 커스텀 이벤트 리스너 추가
    - 이벤트 수신 시 `onSendMessage()`를 직접 호출하여 자동 전송
    - `app/page.tsx`의 `handleQuickInput` 함수 단순화 (문자열만 처리)
    - Quick Question을 시스템 메시지 + 사용자 메시지 구조에서 단일 사용자 메시지로 단순화 (프롬프트 + 클립보드 결합)
  - 기술적 세부사항:
    - Quick Question: 프롬프트와 클립보드 내용을 `\n\n`로 구분하여 하나의 사용자 메시지로 전송
    - 이벤트 기반 통신: IPC → Custom Event → Auto-send의 3단계 흐름
    - 타입 안전성: TypeScript 타입 체크를 통해 메시지 구조 검증
  - 영향: Spotlight와 같은 매끄러운 UX, Quick Input/Question의 실사용성 대폭 향상
  - 수정된 파일:
    - `electron/ipc/handlers/quick-input.ts:59-103` - Quick Question 메시지 결합 로직
    - `components/chat/unified/UnifiedChatInput.tsx:282-308` - Auto-send 이벤트 처리
    - `app/page.tsx:34-59` - Quick Input 처리 단순화
    - `components/chat/ChatContainer.tsx:92-111` - 시스템 메시지 처리 제거
