...
- **근본 원인**: undici 패키지가 `node:` protocol import를 사용하는데, webpack이 클라이언트 번들에 포함시키려고 시도
- **해결 과정**:
  1. 초기 시도: webpack resolve.alias 추가 (node: → 일반 모듈 매핑)
     - 20개 이상의 Node.js 내장 모듈 alias 설정
     - 로컬 빌드는 성공했으나 CI 빌드는 계속 실패
  2. 최종 해결: undici를 webpack externals에 추가
     - undici는 Node.js 전용 HTTP 클라이언트이므로 클라이언트 번들에서 완전히 제외
     - browser-stub.ts를 통해 클라이언트에서는 대체 구현 사용
- **영향**: Windows, macOS, Linux 모든 플랫폼에서 빌드 가능

### 프록시 환경 변수 무시 메커니즘 최종 완성

**v0.6.17 이후 추가 수정 (NO_PROXY 적용)**:

- **문제**: v0.6.17에서 undici.Agent를 `dispatcher` 옵션으로 전달했지만 여전히 프록시 사용됨
  - undici는 `dispatcher` 옵션과 관계없이 환경 변수를 먼저 체크
  - `HTTPS_PROXY`, `HTTP_PROXY` 환경 변수가 있으면 내부적으로 ProxyAgent 생성
  - 런타임에 환경 변수를 삭제해도 이미 캐싱된 값 사용

- **해결**: `NO_PROXY="*"` 환경 변수 설정으로 모든 호스트에 대해 프록시 무시
  - undici는 환경 변수 체크 시 NO_PROXY를 **가장 먼저** 확인
  - `NO_PROXY="*"` 설정 시 `HTTPS_PROXY`, `HTTP_PROXY` 무시
  - fetch 완료 후 원래 NO_PROXY 값으로 복원

- **lib/http/fetch.ts** (fetchWithRetry 함수):

  ```typescript
  // 프록시 비활성화 시
  process.env.NO_PROXY = '*'; // 모든 호스트에 대해 프록시 무시
  process.env.no_proxy = '*';
  delete process.env.HTTP_PROXY; // 이중 방어
  delete process.env.HTTPS_PROXY;
  ```

- **효과**:
  - ✅ **완벽한 프록시 무시**: undici 환경 변수 캐싱 완전 우회
  - ✅ **삼중 방어 메커니즘**:
    1. UI 레벨: 논리적 모순 방지 (v0.6.16)
    2. Agent 레벨: undici.Agent 사용 (v0.6.17)
    3. 환경 변수 레벨: NO_PROXY="\*" 설정 ← **최종 해결 (v0.6.18)**
