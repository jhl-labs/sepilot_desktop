...
- ✅ 타입 안전성 강화
- ✅ Terminal Agent LLM 스트리밍 적용 (Browser Agent와 동일한 방식)
  - 자연어 입력 시 AI가 실시간으로 응답 생성
  - LLMService.streamChatWithChunks() 사용
  - IPC를 통한 스트리밍 청크 전송
...
- ✅ 블록 필터링 로직 추가
- ✅ `window.electronAPI.invoke is not a function` 에러 수정
  - preload.ts에 executeCommand, aiCommand 메서드 추가
  - TerminalPanel.tsx에서 올바른 IPC 호출 방식으로 변경
