...
- `feat(settings)`: NetworkSettingsTab에 환경 변수 디버깅 도구 추가
  - HTTP/HTTPS 통신 관련 환경 변수 표시 기능 추가
  - 실시간으로 환경 변수 값 확인 가능 (HTTPS_PROXY, NODE_EXTRA_CA_CERTS 등)
  - IPC 핸들러 `get-network-env-vars` 추가
  - 테이블 형태로 변수명과 값 표시, 새로고침 버튼 제공
  - 프록시, SSL 인증서 등 네트워크 디버깅 시 유용
  - 다국어(i18n) 지원 추가: 환경 변수 섹션 번역 키 적용 (ko, en, zh)

- `fix(app)`: 단일 인스턴스 실행 및 트레이 아이콘 중복 생성 방지
  - `app.requestSingleInstanceLock()`을 사용하여 단일 인스턴스만 실행되도록 제한
  - 두 번째 인스턴스 실행 시 첫 번째 인스턴스의 창으로 포커스 이동
  - 트레이 아이콘 중복 생성 방지 로직 추가 (이미 생성된 경우 스킵)
  - 앱 종료 시 트레이 아이콘 정리 (`tray.destroy()`)
  - 백그라운드 프로세스 중복 실행 문제 해결
...
- `fix(i18n)`: settings.sync 섹션 추가로 GitHubSyncSettings 번역 지원
  - settings.sync.settings, personas, images, conversations 번역 키 추가
  - 동기화 항목의 제목, 설명, 경고 메시지 다국어 지원
  - 세 언어(한국어, 영어, 중국어) 모두 GitHubSync 동기화 항목 번역 완료

- `fix(i18n)`: EditorSettings 컴포넌트 다국어 지원 추가
...
- `fix(i18n)`: MCPServerList 컴포넌트 다국어 지원 추가
  - settings.mcp.serverList에 toolsUnit, tooltips.activate/deactivate 키 추가
  - zh.json에 전체 serverList 섹션 추가 (이전에 누락됨)
