...
- `fix(editor)`: EditorAgent 파일 작업 및 무한 루프 문제 해결
  - EditorAgent에 MCP 도구 통합 추가 (GitHub API 등 외부 도구 사용 가능)
  - `getEditorTools` 메서드에서 MCP 서버의 도구 자동 로드
  - `executeTool` 메서드에서 MCP 도구 실행 지원
  - 무한 루프 방지 로직 추가: 같은 도구가 동일한 인자로 3번 연속 호출 시 자동 중단
  - 도구 호출 이력 추적 및 중복 감지 메커니즘 구현
  - MCPServerManager에 `listServersInMainProcess()`, `getServerTools()` 메서드 추가
