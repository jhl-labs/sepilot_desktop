...
- **PPT Agent Tool 시스템 구현 및 언어 감지 개선**
  - **Built-in Tool 시스템 추가**:
    - 9개의 PPT 전용 Tool로 구조화된 작업 수행:
      - `generate_presentation`: 새 프레젠테이션 생성 (주제, 언어, 슬라이드 수, 톤, 대상 청중, 디자인 스타일 지정)
      - `modify_slide`: 특정 슬라이드 수정 (제목, 내용, 레이아웃, 색상 등)
      - `add_slide`: 원하는 위치에 슬라이드 추가
      - `delete_slide`: 슬라이드 삭제
      - `reorder_slides`: 슬라이드 순서 변경
      - `translate_presentation`: 전체 프레젠테이션 다른 언어로 번역
      - `change_design_theme`: 디자인 테마 변경 (색상, 폰트, 스타일)
      - `add_chart_to_slide`: 슬라이드에 차트 추가 (bar, line, pie 등)
      - `suggest_improvements`: 프레젠테이션 분석 및 개선 제안
    - Tool 실행 엔진 구현 (`executePptTool`)
    - 실시간 Tool 실행 상태 UI 표시 (✅ 성공, ❌ 실패)
  - **언어 감지 및 강제 적용**:
    - 사용자 메시지에서 자동 언어 감지 (한국어, 영어, 일본어, 중국어)
    - 감지된 언어로 모든 슬라이드 내용 강제 생성
    - "한국어로 만들어줘"라고 해도 영어로 생성되던 문제 해결
    - 언어별 System Prompt 및 Outline Prompt 분리
  - **대화형 슬라이드 수정**:
    - "첫 번째 슬라이드 제목 바꿔줘" 같은 자연어로 슬라이드 수정 가능
    - 기존 슬라이드 상태를 Tool에 전달하여 정확한 수정 가능
  - **Tool 호출 시각화**:
    - `<tool_call>` 태그를 회색 박스로 렌더링
    - Tool 실행 결과 메시지를 색상으로 강조 (성공: 녹색, 실패: 빨간색)
    - 실시간 Tool 실행 피드백 제공
...
- **Editor 모드 추가 (Chat + Editor 듀얼 모드)**
  ...
  - **Editor Chat Agent 모드 선택 기능**
    - Sidebar 헤더에 Agent 모드 토글 버튼 추가 (Bot 아이콘)
    - Chat 모드일 때만 표시되며, "새 대화" 아이콘 우측에 배치
    - **두 가지 Agent 모드 선택 가능**
      - **Editor Agent 모드**: 파일 조작, 코드 어시스턴스, 자동완성에 특화
        - 파일 읽기/쓰기, Tab 관리, Git 통합
        - 코드 분석 도구 (get_file_context, search_similar_code, find_definition)
        - 빠른 응답과 경량 작업에 최적화
        - 최대 50회 반복
      - **Coding Agent 모드**: 복잡한 코딩 작업, 계획 수립, 검증 파이프라인 지원
        - ReAct 패턴: Think → Act → Observe 반복
        - Triage 시스템: 간단한 작업은 직접 응답, 복잡한 작업은 전체 파이프라인 실행
        - 계획 수립 (Planning Pipeline): 단계별 실행 계획 생성
        - 파일 변경 추적 (File Tracker): 작업 중 변경된 파일 자동 추적
        - 검증 파이프라인 (Verification): 실행 결과 자동 검증
        - 에러 복구 (Error Recovery): 실패 시 재시도 로직
        - 웹 검색 지원: 문서 및 예제 검색
        - 최대 10회 반복 (기본값)
    - **Agent 모드 상태 관리**
      - chat-store에 editorAgentMode 상태 추가 ('editor' | 'coding')
      - localStorage에 선택한 Agent 모드 자동 저장
      - 앱 재시작 후에도 마지막 선택한 모드 유지
    - **동적 thinkingMode 전환**
      - Editor 모드 선택 시: thinkingMode = 'editor-agent'
      - Coding 모드 선택 시: thinkingMode = 'coding'
      - 각 모드에 맞는 시스템 메시지 자동 적용
    - **Working Directory 공유**
      - 두 Agent 모두 동일한 working directory 사용
      - SidebarEditor에서 설정한 working directory 자동 전달
    - **UI 레이아웃**
      - Chat 모드: Plus(새 대화) → Bot(Agent 모드) → FolderOpen(파일 탐색기)
      - Files/Search 모드: MessageSquare(Chat) → FolderOpen(파일 탐색기) → Search(전체 검색)
      - 전체 검색 버튼은 Chat 모드에서 숨겨짐 (파일 탐색기 모드에만 표시)
    - **사용 시나리오**
      - 간단한 파일 수정, 코드 읽기, 검색 → Editor Agent
      - 복잡한 멀티파일 리팩토링, 기능 추가, 버그 수정 → Coding Agent
  ...
  - **FileExplorer 컨텍스트 메뉴 및 파일 관리 기능 추가**
    ...
    - **수동 새로고침**
      - Sidebar 헤더에 새로고침 버튼 추가 (Files 모드일 때만 표시)
      - 파일 탐색기 아이콘 우측에 RefreshCw 아이콘 배치
      - 컨텍스트 메뉴에 새로고침 옵션 추가 (단축키: F5)
        - 파일/폴더 우클릭 메뉴 하단
        - 빈 공간 우클릭 메뉴 하단
      - chat-store에 fileTreeRefreshTrigger 상태 추가
      - refreshFileTree() 액션으로 즉시 파일 목록 갱신
      - timestamp 기반 trigger 패턴으로 구현
  - 새로운 IPC 핸들러 추가:
...
- **활성 파일 컨텍스트 추가**
  - Editor Agent가 현재 열려 있는 파일 탭 정보를 자동으로 인식
  - 사용자가 파일명을 명시하지 않으면 현재 활성 탭을 자동으로 대상으로 가정
  - 3가지 시나리오 지원:
    1. **활성 탭 있음**: 파일명, 경로, 언어, 저장 상태를 시스템 프롬프트에 포함하고 IMPORTANT 지시문 추가
    2. **탭은 열려있지만 활성화 안됨**: 모든 열린 탭 목록 표시 및 파일 지정 요청
    3. **열린 탭 없음**: 간단한 안내 메시지
  - 더 직관적인 파일 수정 워크플로우 제공
  - Editor Agent와 Coding Agent 모두에 적용
...
### 모드별 Tool 필터링 및 이미지 생성 기능 제한

- **각 모드에 맞는 Tool만 표시**
  - **Browser 모드**: browser*, google* tool만 표시
    - 페이지 탐색 (browser_navigate)
    - 페이지 검사 (browser_get_page_content, browser_get_interactive_elements 등)
    - 페이지 상호작용 (browser_click_element, browser_type_text, browser_scroll)
    - 탭 관리 (browser_list_tabs, browser_create_tab, browser_switch_tab, browser_close_tab)
    - Vision 기반 tool (browser_capture_annotated_screenshot, browser_click_coordinate 등)
    - Google 검색 tool (google_search, google_search_news, google_search_scholar 등)
    - 웹 검색/fetch tool (webSearch, webFetch)
  - **Editor 모드**: file*, command*, grep\* tool만 표시
    - 파일 작업 (file_read, file_write, file_edit, file_list)
    - 명령 실행 (command_execute)
    - 코드 검색 (grep_search, grepSearch)
  - **Main Chat 모드**: MCP 서버 tool만 표시
    - browser, google, file, command, grepSearch 제외
    - 사용자가 추가한 MCP 서버의 tool만 사용
    - 일반적인 대화 및 외부 API 통합에 집중

- **이미지 생성 기능은 Main Chat 모드에서만 활성화**
  - Browser/Editor 모드에서는 ImageGen 토글 버튼 숨김
  - ImageGen 드롭다운 메뉴 (provider 선택) 숨김
  - ImageGenerationProgressBar는 기존에 이미 Main Chat 전용

**변경된 파일**:

- `components/chat/unified/UnifiedChatInput.tsx`: tool 필터링 로직 추가, 이미지 생성 UI mode 조건 추가

**기술적 배경**:

- 각 agent는 특정 domain의 tool만 사용해야 함
  - Browser agent는 웹 브라우저 제어에 집중
  - Editor agent는 파일 시스템 및 코드 작업에 집중
  - Main Chat agent는 외부 MCP 서버 통합 및 일반 대화에 집중
- tool 필터링은 `window.electronAPI.mcp.getAllTools()`의 결과를 mode 기반으로 필터링
- 이미지 생성은 Main Chat의 크리에이티브 기능으로 분류

**영향**:

- 각 모드에서 관련 있는 tool만 표시되어 혼란 감소
- Browser/Editor 모드에서 불필요한 이미지 생성 버튼 제거
- UI가 더 깔끔하고 목적에 맞게 최적화됨

---

### Coding Agent 유틸리티 통합 및 Context Management 개선

**목적**:

- Coding Agent의 실행 품질 향상
- Tool 사용 패턴 분석 및 최적화
- 파일 변경 추적 및 롤백 기능
- 코드베이스 구조 분석 및 관련 파일 추천
- 정확한 토큰 계산 및 컨텍스트 관리

**주요 개선사항**:

1. **ToolSelector 통합**
   - Tool 사용 통계 자동 수집 (성공률, 평균 실행 시간, 에러 패턴)
   - 중복 Tool 호출 감지 및 경고
   - Tool 최적화 제안 (배치 처리, 캐싱, 대안 제시)
   - 24시간 통계 보존으로 세션 간 학습

2. **FileTracker 통합**
   - 파일 수정 전후 스냅샷 자동 생성
   - SHA-256 기반 파일 변경 감지
   - Diff 자동 생성 (변경 내용 추적)
   - Iteration별 롤백 포인트 생성
   - 최근 10개 스냅샷 유지 (파일당)
   - 최근 5개 롤백 포인트 유지

3. **CodebaseAnalyzer 통합**
   - 프로젝트 구조 자동 분석 (파일 트리, 통계)
   - Import/Export 추출로 의존성 그래프 구축
   - 사용자 요청 기반 관련 파일 자동 추천
   - Planning 단계에서 필요한 파일 사전 식별
   - node_modules, .next, dist 등 자동 제외

4. **Context Management 개선**
   - Tiktoken 기반 정확한 토큰 계산 (GPT-4 tokenizer 사용)
   - 기존: 문자 수 기반 추정 (4 chars/token) → 새로운: 실제 토큰 계산
   - 메시지 압축 기능 추가 (핵심 정보만 추출)
   - 토큰 사용 통계 로깅 (total, utilization%, remaining)
   - 100k 토큰 윈도우 내에서 최적화된 메시지 선택
   - System 메시지, 최근 메시지, 중요 Tool 결과 우선 보존

**기술적 구현**:

- 모든 유틸리티 클래스를 static 멤버로 공유 (세션 간 통계 유지)
- `CodingAgentGraph.toolSelector`, `fileTracker`, `codebaseAnalyzer`, `contextManager`
- Tool 실행 전후로 자동 추적 (file_write, file_edit 작업)
- 에러 발생 시에도 통계 기록으로 패턴 학습
- Planning 단계에서 codebase 분석 결과 활용

**변경된 파일**:

- `lib/langgraph/graphs/coding-agent.ts`: 유틸리티 통합, Context Manager 사용
- `lib/langgraph/utils/context-manager.ts`: Tiktoken 통합, 압축 기능 추가
- `lib/langgraph/utils/tool-selector.ts`: (기존)
- `lib/langgraph/utils/file-tracker.ts`: (기존)
- `lib/langgraph/utils/codebase-analyzer.ts`: (기존)

**성능 영향**:

- Tool 중복 호출 방지로 불필요한 실행 감소
- 관련 파일 자동 추천으로 탐색 시간 단축
- 정확한 토큰 계산으로 컨텍스트 오버플로우 방지
- 파일 변경 추적으로 디버깅 및 롤백 용이
- Agent가 잘못된 tool을 사용할 가능성 감소

### 도구 실행 승인 메시지 중복 표시 버그 수정

- **승인 대기 메시지 중복 방지**
  - "🔔 도구 실행 승인을 기다리는 중..." 메시지가 두 번 나타나는 문제 해결
  - tool_approval_request 이벤트 처리 시 메시지 존재 여부 확인

- **구현 내용**
  - accumulatedContent.includes()로 메시지 중복 체크
  - 이미 같은 메시지가 있으면 추가하지 않음
  - Main Chat (useMessageStreaming) 및 Editor Chat (EditorChatContainer) 모두 적용

**변경된 파일**:

- `components/chat/unified/hooks/useMessageStreaming.ts`: 승인 메시지 중복 체크 추가
- `components/editor/EditorChatContainer.tsx`: 승인 메시지 중복 체크 추가

**기술적 배경**:

- tool_approval_request 이벤트가 처리될 때 무조건 메시지를 추가하던 방식
- 같은 이벤트가 재처리되거나 UI 업데이트가 중복되면 메시지도 중복됨
- 메시지 존재 여부를 체크하여 중복 방지

**영향**:

- Tool 승인 대기 중 UI가 깔끔해짐
- 중복 메시지로 인한 혼란 제거
- Human-in-the-loop 사용자 경험 개선

### 이미지 생성 활성화 시 Tools 비활성화 방지

- **이미지 생성 중 Tools 설정 보호**
  - enableImageGeneration이 true일 때 enableTools를 false로 변경 차단
  - 이미지 생성은 generate_image tool에 의존하므로 tools 필수

- **구현 내용**
  - setEnableTools()에서 enableImageGeneration 상태 확인
  - 이미지 생성 활성화 상태에서는 tools 비활성화 무시

**변경된 파일**:

- `lib/store/chat-store.ts`: setEnableTools에 보호 로직 추가

**기술적 배경**:

- generate_image는 MCP tool로 구현됨
- enableTools가 false면 tool 호출 자체가 불가능
- 이미지 생성 중 tools를 끄면 실행 실패

**영향**:

- 이미지 생성 실패 방지
- 사용자가 실수로 설정을 잘못 변경하는 것 방지
- 설정 간 의존성 명확화

### 새 대화 생성 시 기본값을 Instant 모드로 설정

- **새 대화는 가장 빠르고 가벼운 Instant 모드로 시작**
  - thinkingMode: 'instant'
  - enableRAG: false (RAG 문서 검색 비활성화)
  - enableTools: false (MCP Tools 비활성화)
  - enabledTools: [] (선택된 tool 없음)
  - enableImageGeneration: false (이미지 생성 비활성화)

- **사용자가 필요에 따라 기능 활성화 가능**
  - 새 대화에서 RAG/Tools/이미지 생성을 원하면 수동으로 활성화
  - 각 대화마다 독립적인 설정 유지
  - 대화 전환 시 해당 대화의 저장된 설정 자동 복원
...
- `lib/store/chat-store.ts`: createConversation 함수에서 기본값 설정

**기술적 배경**:

- Instant 모드는 가장 빠른 응답 속도 제공
- RAG 문서 검색과 Tool 호출은 추가 지연 발생
- 대부분의 간단한 질문은 Instant 모드로 충분
- 필요한 경우에만 고급 기능 활성화

**영향**:

- 새 대화 생성 시 더 빠른 응답 속도
- 불필요한 RAG/Tool 호출로 인한 비용 절감
- 초보 사용자도 간단하게 시작 가능
- 고급 사용자는 필요에 따라 기능 활성화
