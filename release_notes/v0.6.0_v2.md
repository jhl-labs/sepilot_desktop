...
- **GraphFactory 라우팅 개선**:
  - streamWithConfig에서 Deep Web Research를 전용 메서드로 처리
  - enableTools 플래그와 무관하게 자체 도구 관리 흐름 사용
  - streamDeepWebResearchGraph 메서드 추가
  - Completion 이벤트 emit으로 UI 로딩 상태 자동 정리
  - "AI가 응답을 생성하고 있습니다..." 메시지가 답변 완료 후에도 남는 문제 해결

### Chat Agent UI 로딩 상태 개선

- **Completion 이벤트 추가**:
  - Agent가 정상 종료될 때 completion 이벤트 emit
  - UI가 로딩 상태를 정리할 수 있도록 신호 제공
  - 답변 완료 후에도 "AI가 응답을 생성하고 있습니다..." 메시지가 남는 문제 해결

- **InputBox completion 이벤트 처리**:
  - event.type === 'completion' 처리 로직 추가
  - Agent 완료 시 nodeStatus 자동 초기화
  - 로딩 표시가 정상적으로 사라져 사용자 혼란 방지

- **LangGraphCompletionEvent 타입 추가**:
  - types/electron.d.ts에 completion 이벤트 타입 정의
  - LangGraphStreamEvent union에 추가
  - iterations 필드 포함 (선택적)

- **Tool Approval Dialog conversationId 필터링**:
  - EditorChatInput, SimpleChatInput에서 자신의 conversationId 이벤트만 처리
  - 다른 대화의 tool_approval_request 이벤트 간섭 방지
  - InputBox의 Tool Approval Dialog가 정상 표시되도록 수정
