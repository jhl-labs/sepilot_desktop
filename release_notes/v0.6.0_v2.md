...
- 사용자가 승인/거부 후 즉시 다음 작업 진행 가능
- Human-in-the-loop 기능 사용성 개선

### Ollama 사용 시 대화 제목 생성 오류 수정

- **Ollama provider 사용 시 401 Unauthorized 에러 해결**
  - Ollama는 API 키가 필요 없는데 OpenAI 형식의 Authorization 헤더를 보내서 발생한 문제 수정
  - 에러 메시지: "Incorrect API key provided: ollama"

- **구현 내용**
  - `lib/llm/http-utils.ts`의 `createAuthHeader` 함수 개선
  - Ollama provider 감지 시 빈 헤더 객체 반환
  - OpenAI, Anthropic 등 다른 provider는 기존 로직 유지

**변경된 파일**:

- `lib/llm/http-utils.ts`: createAuthHeader에 Ollama provider 처리 로직 추가

**기술적 배경**:

- Ollama는 로컬 LLM 서버로 API 키 인증이 불필요함
- OpenAI 호환 API를 제공하지만 Authorization 헤더를 보내면 401 에러 발생
- 대화 제목 생성 기능(`llm-generate-title`)이 현재 설정된 provider를 사용하므로 영향 받음

**영향**:

- Ollama 사용자가 대화 제목 자동 생성 기능 정상 사용 가능
- llama3.2, deepseek 등 로컬 모델 사용 시 안정성 향상
