...
export function useFileSystem(): UseFileSystemReturn {
    ...
    async (filePath: string): Promise<string | null> => {
      ...
      try {
        ...
        if (result.success) {
          ...
          console.log(
            `[FileSystem] File read successfully: ${filePath} (${result.data?.length || 0} bytes)`
          );
          return result.data || '';
        } else {
  ...
  const readDirectory = useCallback(
    async (dirPath: string): Promise<any[] | null> => {
      if (!isAvailable) {
        console.warn('[FileSystem] Not available in browser mode');
        return null;
      }
      ...
      try {
        ...
        if (result.success && result.data) {
          console.log(
            `[FileSystem] Directory read successfully: ${dirPath} (${result.data.length} items)`
          );
          return result.data;
        } else {
