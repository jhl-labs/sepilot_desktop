...
export function LLMSettingsTab({
  ...
  const [autocompleteModelFetchError, setAutocompleteModelFetchError] = useState<string | null>(
    null
  );
  ...
  const handleRefreshAutocompleteModels = async () => {
    ...
    try {
      ...
      setAutocompleteModelFetchError(
        error.message || 'Autocomplete 모델 정보를 불러오지 못했습니다.'
      );
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {modelFetchError && <p className="text-xs text-destructive">{modelFetchError}</p>}
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                이미지 이해/해석이 필요한 멀티모달 요청 시 사용할 Vision 모델을 별도로 지정할 수
                있습니다.
              </p>
            </div>
          ...
          {config.vision?.enabled && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      isVisionModelLoading || (!config.vision?.apiKey && !config.apiKey.trim())
                    }
                    title="Vision 모델 목록 새로고침"
                  >
                    <RefreshCw
                      className={`h-4 w-4 ${isVisionModelLoading ? 'animate-spin' : ''}`}
                    />
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Vision 모델에 다른 API 키를 사용하려면 입력하세요. 비워두면 기본 LLM API 키를
                  사용합니다.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <option value="__vision_custom__" className="bg-background text-foreground">
                    직접 입력
                  </option>
                </select>
                {isVisionModelLoading && (
                  <p className="text-xs text-muted-foreground">
                    Vision 모델 목록을 불러오는 중입니다...
                  </p>
                )}
                ...
                {(config.vision?.apiKey || config.apiKey.trim()) &&
                  visionModelOptions.length === 0 &&
                  !isVisionModelLoading &&
                  !visionModelFetchError && (
                    <p className="text-xs text-muted-foreground">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  LiteLLM + Ollama 비전 모델 조합에서는 스트리밍 파싱 오류가 발생할 수 있습니다.
                  기본값(비활성화) 권장.
                </p>
              </div>
            </div>
        ...
        <div className="pt-6 border-t space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                에디터에서 코드 자동완성에 사용할 모델을 별도로 지정할 수 있습니다. (빠른 응답을
                위해 가벼운 모델 권장)
              </p>
            </div>
            <label className="relative inline-flex items-center cursor-pointer">
          ...
          {config.autocomplete?.enabled && (
            <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      (!config.autocomplete?.apiKey && !config.apiKey.trim())
                    }
                    title="자동완성 모델 목록 새로고침"
                  >
                    <RefreshCw
                      className={`h-4 w-4 ${isAutocompleteModelLoading ? 'animate-spin' : ''}`}
                    />
                  </Button>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  자동완성 모델에 다른 API 키를 사용하려면 입력하세요. 비워두면 기본 LLM API 키를
                  사용합니다.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <option value="__autocomplete_custom__" className="bg-background text-foreground">
                    직접 입력
                  </option>
                </select>
                {isAutocompleteModelLoading && (
                  <p className="text-xs text-muted-foreground">
                    자동완성 모델 목록을 불러오는 중입니다...
                  </p>
                )}
                {autocompleteModelFetchError && (
                  <p className="text-xs text-destructive">{autocompleteModelFetchError}</p>
                )}
                {!config.autocomplete?.apiKey &&
                  !config.apiKey.trim() &&
                  !isAutocompleteModelLoading && (
                    <p className="text-xs text-muted-foreground">
        ...
        <div className="pt-6 border-t space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              LLM API 호출 시 전송할 커스텀 HTTP 헤더를 설정합니다. (다른 서비스에는 적용되지
              않습니다)
            </p>
          </div>
