...
const DEFAULT_REFINEMENT_PROMPT = `당신은 문서를 정제하고 핵심 내용을 추출하는 전문가입니다.
...
export function DocumentUploader({ onUpload, disabled = false }: DocumentUploaderProps) {
  ...
  const executeRefinement = async (customPrompt: string) => {
    setIsFetching(true);
    setMessage(null);
    ...
    try {
      ...
      setContent(result.data.content);
      setRefinementPrompt(customPrompt); // 사용자가 수정한 프롬프트 저장
  ...
  return (
    <>
      <RefinementPromptDialog
        open={showPromptDialog}
        onOpenChange={setShowPromptDialog}
        defaultPrompt={refinementPrompt}
        content={content}
        onConfirm={executeRefinement}
      />
