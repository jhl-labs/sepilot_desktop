...
export function LLMStatusBar({
      ...
      try {
        ...
        if (result.success && result.data?.llm) {
          if (isLLMConfigV2(result.data.llm)) {
            ...
            const baseModels = result.data.llm.models.filter((m: any) => m.tags.includes('base'));
      ...
      setTimeout(async () => {
        if (isElectron() && window.electronAPI) {
          ...
          if (reloaded.success && reloaded.data?.llm && isLLMConfigV2(reloaded.data.llm)) {
            ...
            const baseModels = reloaded.data.llm.models.filter((m: any) => m.tags.includes('base'));
            setAvailableModels(baseModels);
            console.log(
  ...
  const saveEditedField = () => {
    ...
    console.log('[LLMStatusBar] Saving edited field:', { field: editingField, value: newValue });
    setEditingField(null);
    onConfigUpdate?.(updatedConfig);
  };
