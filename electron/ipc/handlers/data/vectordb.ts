...
import { VectorDocument, SearchResult, ExportData } from '@/lib/domains/rag/types';
...
export function setupVectorDBHandlers() {
  ...
  ipcMain.handle('vectordb-get-all', async () => {
    try {
      const documents = await vectorDBService.getAllDocuments({ includeEmbedding: false });
  ...
  ipcMain.handle('vectordb-export', async () => {
    try {
      ...
      const documents = await vectorDBService.getAllDocuments({ includeEmbedding: true });
  ...
  ipcMain.handle(
    ...
    async (_, exportData: ExportData, options?: { overwrite?: boolean }) => {
      try {
        ...
        // Import 헬퍼 함수 사용
        const { processImportDocuments } = await import('@/lib/domains/rag/import-helper');

        const result = await processImportDocuments(
          exportData,
          {
            getAllDocuments: () => vectorDBService.getAllDocuments({ includeEmbedding: false }),
            deleteDocuments: (ids: string[]) => vectorDBService.delete(ids),
            insertDocuments: (docs: VectorDocument[]) => vectorDBService.insert(docs),
          },
          { overwrite }
        );

        logger.debug('Documents imported successfully', result);
  ...
  ipcMain.handle('vectordb-get-all-empty-folders', async () => {
    try {
      ...
      const allDocs = await vectorDBService.getAllDocuments({ includeEmbedding: false });
