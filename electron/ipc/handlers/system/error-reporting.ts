...
import os from 'os';
import { loadAppConfig } from '../../../services/secure-config';
 ...
 */
async function validateGitHubSyncConfig(): Promise<GitHubSyncConfig> {
...
export function setupErrorReportingHandlers() {
  ...
  ipcMain.handle('error-reporting-send', async (_event, errorData: ErrorReportData) => {
    try {
      // GitHub Sync 설정 검증
      const githubSync = await validateGitHubSyncConfig();
