...
interface BrowserTab {
  ...
  setupListeners?: () => void; // Optional: for future background tab optimization
}
...
function createBrowserView(
  ...
  // Increase max listeners to avoid warnings when multiple tabs are open
  // Each tab has ~10 event listeners, which is normal for browser tabs
  ...
  ipcMain.handle('browser-view:close-tab', async (event, tabId: string) => {
    try {
      ...
      if (activeTabId === tabId) {
        ...
        const remainingTabs = Array.from(tabs.values()).filter((t: any) => t.id !== tabId);
