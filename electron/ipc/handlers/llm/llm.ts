...
import { getLLMClient, initializeLLMClient } from '@/lib/domains/llm/client';
import { hasImages, createVisionProvider } from '@/lib/domains/llm/vision-utils';
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        // Initialize LLM client with autocomplete config
        const { initializeLLMClient } = await import('@/lib/domains/llm/client');
        ...
        try {
          ...
          if (
            editorAgentGraph &&
            typeof (editorAgentGraph as any).initializeLLMClient === 'function'
          ) {
            (editorAgentGraph as any).initializeLLMClient(providerConfig);
            logger.info('[EditorAgent/Autocomplete] Extension LLM client initialized');
          }
        } catch (extError) {
          logger.warn(
            '[EditorAgent/Autocomplete] Failed to initialize extension LLM client:',
            extError
          );
        }
        ...
        // Word wrap 설정에 따른 프롬프트 추가
        const wordWrapColumn = context.metadata?.wordWrapColumn || 80;
        const wordWrapInstruction = `\n- Each line should be maximum ${wordWrapColumn} characters
- Break lines naturally at word boundaries when approaching ${wordWrapColumn} characters`;
...
- Match the writing style and context${wordWrapInstruction}${context.useRag ? '\n- You can reference uploaded documents if needed' : ''}${context.useTools ? '\n- You can use available tools if needed' : ''}`,
        ...
        logger.info('[EditorAgent/Autocomplete] Calling LLM directly (no tool calling needed)');

        // Use direct LLM call for autocomplete (no tool calling needed)
        ...
        const response = await provider.chat(
          ...
          })),
          {
            ...
            maxTokens: providerConfig.maxTokens || 500,
          }
        );

        let completion = response.content || '';
        logger.info('[EditorAgent/Autocomplete] Got completion:', completion.substring(0, 100));
    ...
    async (
      ...
      try {
        ...
        try {
          ...
          if (
            editorExtension &&
            typeof (editorExtension as any).initializeLLMClient === 'function'
          ) {
            (editorExtension as any).initializeLLMClient(config.llm);
            logger.info('[EditorAgent/Action] Extension LLM client initialized');
          }
        } catch (extError) {
          logger.warn('[EditorAgent/Action] Failed to initialize extension LLM client:', extError);
        }
...
Return ONLY the result of the instruction.`;
        ...
        // Simple actions (writing tools) don't need GraphFactory or tool calling
        // Use direct LLM call for better performance and reliability
        const simpleActions = [
          'continue',
          'make-shorter',
          'make-longer',
        ...
        const isSimpleAction = simpleActions.includes(params.action);

        // Word wrap 설정
        const wordWrapColumn = params.context?.wordWrapColumn || 80;
        const wordWrapInstruction = `\n- Each line should be maximum ${wordWrapColumn} characters where possible\n- Break lines naturally at word boundaries when approaching ${wordWrapColumn} characters`;

        // Add word wrap instruction to system prompt for code actions
        if (!isSimpleAction) {
          systemPrompt += wordWrapInstruction;
        }

        let result = '';

        if (isSimpleAction) {
          // Direct LLM call for simple actions (no tool calling needed)
          logger.info('[EditorAgent/Action] Using direct LLM call for simple action');
          ...
          // Use GraphFactory for code actions that may need tool calling
          logger.info('[EditorAgent/Action] Using GraphFactory for code action');
