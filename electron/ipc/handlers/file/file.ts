...
// sharp는 네이티브 모듈이므로 lazy-load (사용 시점에 require)
...
async function processImage(filePath: string): Promise<Buffer> {
  try {
    // sharp를 사용 시점에 동적으로 로드 (네이티브 모듈 lazy loading)
    let sharp;
    try {
      sharp = require('sharp');
    } catch (sharpError) {
      console.warn('[File] sharp module not available, returning original image:', sharpError);
      // sharp가 없으면 원본 이미지 반환
...
export function registerFileHandlers() {
  ...
  // 파일 다른 이름으로 저장 다이얼로그
  ipcMain.handle('file:save-as', async (_event, content: string, defaultFilename?: string) => {
    try {
      const focusedWindow = BrowserWindow.getFocusedWindow();
      if (!focusedWindow) {
        return {
      ...
      console.log('[File] File saved successfully via Save As:', filePath);
